...
class User_api_model extends CI_Model {
    ...
    public function get_user_public_profile_count($user_id) {
        ...
        if ($user -> current_city_id > 0) {
            $public_profile_count++;
        }

        if (strlen($user -> birth_date) > 0) {
            $public_profile_count++;
        }
        ...
        if ($user -> religious_belief_id > 0) {
            $public_profile_count++;
        }

        if ($user -> relationship_status_id > 0) {
            $public_profile_count++;
        }

        if ($user -> body_type_id > 0) {
            $public_profile_count++;
        }

        return $public_profile_count;
    }

    public function get_user_experience_count($user_id) {

        $user = $this -> get_user($user_id);

        $CI = &get_instance();

        $CI->load->model('api/av2/user_education_level_api_model');
        $CI->load->model('api/av2/user_school_api_model');
        $CI->load->model('api/av2/user_job_api_model');

        $experience_count = 0;

        $education_levels = $CI -> user_education_level_api_model -> get_user_education_levels_by_user_id($user_id);
        if (!empty($education_levels) && count($education_levels) > 0) {
            $experience_count++;
        }

        $user_schools = $CI -> user_school_api_model -> get_user_schools_by_user_id($user_id);
        if (!empty($user_schools) && count($user_schools) > 0) {
            $experience_count++;
        }

        $user_jobs = $CI -> user_job_api_model -> get_user_jobs_by_user_id($user_id);
        if (!empty($user_jobs) && count($user_jobs) > 0) {
            $experience_count++;
        }

        if ($user -> annual_income_range_id > 0) {
            $experience_count++;
        }
    ...
    public function get_user_personality_count($user_id) {

        $user = $this -> get_user($user_id);

        $CI = &get_instance();

        $CI->load->model('api/av2/user_descriptive_word_api_model');

        $personality_count = 0;

        if ($user -> smoking_status_id > 0) {
            $personality_count++;
        }

        if ($user -> drinking_status_id > 0) {
            $personality_count++;
        }

        if ($user -> exercise_frequency_id > 0) {
            $personality_count++;
        }

        $descriptive_words = $this -> user_descriptive_word_api_model -> get_user_descriptive_words_by_user_id($user_id);
        if (!empty($descriptive_words) && count($descriptive_words) > 0) {
            $personality_count++;
        }
    ...
    public function find_profile_matches($user_id, $limit = 0) {
        ...
        $CI->load->model('api/av2/user_decision_api_model');
    ...
    public function send_push_notification_to_user($user_id, $message, $meta = '', $debug = FALSE) {
        ...
        if (!empty($user->device_token)) {
            ...
            $this->send_push_notification_to_device($user->device_token, $message, $meta, $debug);
        }

        if (empty($user->device_token_android)) {
            return array(
                'status' => FALSE,
                'detail' => 'Android Registration Token does not exist.'
            );
        } else {
            // Send push notification to Android device
            return $this->send_push_notification_to_android_device($user->device_token_android, $message, $meta, $debug);
        }
    }
    ...
    public function send_push_notification_to_android_device($device_token_android, $message, $meta = '', $debug = FALSE) {

        define("GOOGLE_API_KEY", "AIzaSyAuhtKi_nwDoFGMB_XVYwm-H5mNIZBoZo8");
        ...
        if ($result === FALSE) {
            return array(
                'status' => FALSE,
                'detail' => 'Failed to send ' . curl_error($ch)
            );
        }
        curl_close($ch);
        ...
        return array(
            'status' => TRUE,
            'detail' => 'Android Push Notification is sent successfully.'
        );
    }
