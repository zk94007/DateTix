...
class User_School_api_model extends CI_Model {
	...
	public function delete_user_school_and_links($user_school_id){
		$CI = & get_instance();
		$CI -> load -> model('api/av2/user_school_major_minor_api_model');
	...
	public function get_user_school_details($user_school_id,$language_id = ""){
		$CI = & get_instance();
		$CI -> load -> model('api/av2/user_school_major_minor_api_model');
		$CI -> load -> model('api/av2/school_api_model');
	...
	public function delete_user_schools_by_user_id($user_id) {

		$this -> db -> where('user_id', $user_id);
		$this -> db -> delete('user_school');
	}

	public function delete_excluded_user_schools_by_ids($user_id, $user_school_ids) {

		$this -> db -> where('user_id', $user_id);
		$this -> db -> where_not_in('user_school_id', $user_school_ids);
		$this -> db -> delete('user_school');
	}
}
