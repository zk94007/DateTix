...
class User_Photo_api_model extends CI_Model {
	...
	public function insert_user_photo_with_upload_data($user_id, $data, $set_primary){

		$data = array(
			'user_id'=>$user_id,
			'photo'=>$data['file_name'],
			'uploaded_time'=>SQL_DATETIME
		);
	...
	public function clear_primary_photo_by_user_id($user_id) {
	...
	public function set_primary_photo_with_params($user_id, $user_photo_id) {
		...
		$this -> clear_primary_photo_by_user_id($user_id);
	...
	public function delete_user_photos_by_user_id($user_id) {

		$this -> db -> where('user_id', $user_id);
		$this -> db -> delete('user_photo');
	}

	public function delete_excluded_user_photos_by_ids($user_id, $user_photo_ids) {

		$this -> db -> where('user_id', $user_id);
		$this -> db -> where_not_in('user_photo_id', $user_photo_ids);
		$this -> db -> delete('user_photo');
	}
}
