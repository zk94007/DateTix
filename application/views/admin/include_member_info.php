<div class="userBox-wrap">        
	<div class="userTopRow">
		<div class="userTopRowHed"><?php echo $user['first_name'].' '.$user['last_name'].' ('.$user['user_id'].' - '.$user['completed_application_step'].', '. $user['city_description'] . ')';?>		
			<?php if($user['website_name'] != ""):?>
			(<?php echo $user['website_name'];?>)
			<?php endif;?>
		...
		<?php if(!$is_review_resricted && 1==2):?>
			...
			<div class="sml-img">
                ...
                <div class="userBoxRight">                    
                        <div class="sfp-1-main">
                            <div class="sfp-1-Left bold "><?php echo translate_phrase('Handpick ').$user['first_name'].' '.translate_phrase('For');?></div>
                            <div class="sfp-1-Right ">
                                    <dl class="dropdown-dt domaindropdown common-dropdown">
                                        ...
                                        <a href="javascript:;" key=""><span class="y-overflow-hidden"><?php echo translate_phrase('Please Select'); ?>
                                            </span> </a> <input type="hidden" name="m_handpicked_user_id" id="m_handpicked_user_id" value="">
                                        </dt>
                                        <dd>
                                            <ul>                                                
                                                <?php foreach ($handpickedMembers as $key => $value):?>
                                                    <li>
				...
				<div class="sfp-1-Right">
                                    <div class="userTopRowHed membersMsgArea_<?php echo $user['user_id']?>">
                                        <h4>                        
                                            <span class="DarkGreen-color"></span>                                                                        
                                            <span class="Red-color"></span>                                                                        
                                        </h4>     
                                    </div> 
                                    <button type="button" class="btn btn-blue left" style="height: 36px;" data-user-id="<?php echo $user['user_id'] ?>" onclick="handpickMatch(this,'<?php echo $user['first_name']?>')">
                                        <?php echo translate_phrase("Handpick Match");?>
                                    </button>
                                </div>
			</div>
