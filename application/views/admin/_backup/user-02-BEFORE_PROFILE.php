...
class User extends MY_Controller {
	...
	*/
	...
	/**
	 * - When a user is viewing profile of another user whose matchmaker_id > 0, then don't show chat box and instead show "Date Me" button
		- When the "Date Me" button is clicked, do the following:
		1. Send email to "matchmaker.contact_email_address" with the email subject "Free consultation request" and email body "Michael Ye wants to date with your member Jennifer Chan. You may contact him at 6684-2770 and mikeye27@gmail.com to follow-up."
	 ...
	 */
	 public function date_me()
	 {
		...
		if(!$cur_user_id )
		{
	 	...
	 	$matchmaker_member_user_id = $this->input->post('user_id');
		$response['type'] = 'error';
		$msg = '';
		
		if($matchmaker_member_user_id )
		{
			
			$this->general->set_table('user');
			$matchmaker_id = 0;
			
			if($matchmaker_member_datas = $this->general->get("first_name,last_name,matchmaker_id",array('user_id'=>$matchmaker_member_user_id)))
			{
				$matchmaker_id = $matchmaker_member_datas['0']['matchmaker_id'];
			}
			else {
			...
			if($matchmaker_id)
			{
				...
				if($matchmaker_datas = $this->general->get("",array('matchmaker_id'=>$matchmaker_id)))
				{
					$matchmaker_email = $matchmaker_datas['0']['contact_email_address'];
					...
					$msg = $matchmaker_member_datas['0']['first_name'].translate_phrase(" is a member of ").$matchmaker_datas['0']['company_name'].translate_phrase(". They will contact you soon to arrange a date for you with ").$matchmaker_member_datas['0']['first_name'];
				}
				else {
	...
	public function user_info($view_user_id, $login_user_id = '', $pass = '') {
		...
		$mm = $this->session->userdata('requestedBymm');
		...
		$cur_user_id = $this -> session -> userdata('user_id');
		if (!$user_intros && $user_id != $cur_user_id && $mm === false) {
			if ($cur_user_id) {
