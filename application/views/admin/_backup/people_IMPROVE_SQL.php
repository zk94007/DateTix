...
class People extends MY_Controller {
	...
	private function getPeople($condtion_str, $offset) {
		...
		$sql = "SELECT " . $select_people . " 
			FROM user
			LEFT JOIN user_decision ON user.user_id != user_decision.target_user_id AND user_decision.user_id = '" . $this -> user_id . "' AND user_decision.decision IS NOT NULL
			WHERE " . $condtion_str . " ORDER BY user.last_active_time desc LIMIT " . $offset . ",1";
		//echo $sql;
