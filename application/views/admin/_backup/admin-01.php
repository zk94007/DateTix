...
class Admin extends CI_Controller {
	...
	public function __construct() {
		...
		if ($logged_in === FALSE) {
			...
			if (($_SERVER['PHP_AUTH_USER'] == $user) && ($_SERVER['PHP_AUTH_PW'] == $password)) {
				...
				$this -> general -> set_table('matchmaker');
				...
				//echo '<pre>';print_r($CheckAdmin);die();
				if (!empty($CheckAdmin)) {
					//$this -> session -> set_userdata(array('superadmin_logged_in' => TRUE));
	...
	public function index() {
		...
		$city_select = "select ct.city_id, ct.description from user 
                            ...
                            WHERE approved_date IS NULL GROUP BY current_city_id ORDER BY ct.description";
		$data['cities'] = $this -> general -> sql_query($city_select);

		$ChekCondition['matchmaker_id'] = $LoginData['matchmaker_id'];
		...
		$user_application_sql = "SELECT city.description AS city_description, status.account_status_id, status.description, last_display_language_id,
					user_id,completed_application_step,last_active_time,facebook_id,gender_id,first_name,last_name,num_date_tix,attractiveness_level,facebook_page
					FROM user as usr
		...
		else
		{
			$user_application_sql = "SELECT city.description AS city_description, status.account_status_id, status.description, last_display_language_id,
						user_id,completed_application_step,last_active_time,facebook_id,gender_id,first_name,last_name,num_date_tix,attractiveness_level,facebook_page,
						matchmaker.company_name
						FROM user as usr
						LEFT JOIN matchmaker on usr.matchmaker_id = matchmaker.matchmaker_id
						JOIN account_status as status on usr.account_status_id = status.account_status_id					
						JOIN city on usr.current_city_id = city.city_id
						WHERE 
							usr.approved_date IS NOT NULL AND ";
			
			if ($ChekCondition['matchmaker_id'] == 1)
			{
				$user_application_sql .= "(usr.matchmaker_id = " . $ChekCondition['matchmaker_id'] . " OR usr.matchmaker_id=0) AND ";
			}
			else
			{
				$user_application_sql .= "usr.matchmaker_id = " . $ChekCondition['matchmaker_id'] . " AND ";
			}
		...
		$data['gender'] = $this -> model_user -> get_gender($this -> language_id);
                ...
                $data['income_level'] = $this->model_user->get_annual_income_range($this->language_id);                                
		...
		$data['param'] = $this -> input -> post('param');
                ...
                $SQL = 'SELECT user_id,first_name,last_name FROM user WHERE num_date_tix > 0 ORDER BY first_name ASC,last_name ASC';
	...
	public function load_users($type = 'review_app') {
		...
		if ($this -> input -> is_ajax_request()) {
			...
			$user_application_sql = "SELECT city.description as city_description, status.account_status_id, status.description, last_display_language_id,
						usr.user_id,completed_application_step,last_active_time,facebook_id,usr.gender_id,first_name,last_name,num_date_tix,attractiveness_level,facebook_page,usr.credits_value,
						CASE 
							WHEN
			...
			if ($type == 'marketplace' && $this -> session -> userdata('LIMITED_ACCESS'))
				$user_application_sql .= ", matchmaker.company_name";
			...
			if ($type == 'marketplace' && $this -> session -> userdata('LIMITED_ACCESS'))
				$user_application_sql .= " JOIN matchmaker on usr.matchmaker_id = matchmaker.matchmaker_id";
			...
			if ($type == "manage_member" || $type == 'marketplace') {
				...
				$matchmakerId = $superAdminData['matchmaker_id'];
				...
				if ($type == "manage_member") {
					...
					if ($this -> session -> userdata('LIMITED_ACCESS'))
					{
						if ($matchmakerId == 1)
						{
							$user_application_sql .= " AND (matchmaker_id = " . $matchmakerId . " OR matchmaker_id=0) ";
						}
						else
						{
							$user_application_sql .= " AND `matchmaker_id` =" . $matchmakerId . " AND `matchmaker_id` > 0";
						}
					}
				...
				if ($type == 'marketplace' && $this -> session -> userdata('LIMITED_ACCESS'))
				{
					$user_application_sql .= " AND usr.matchmaker_id !=" . $matchmakerId . " AND usr.matchmaker_id > 0";
					if(isset($find_match_of_userdata['matchmaker_id']) && $find_match_of_userdata['matchmaker_id'])
					{
						$user_application_sql .= " AND usr.matchmaker_id != '" . $find_match_of_userdata['matchmaker_id'] . "'";
					}
				}				
			}
			...
			$usersRes = $this -> general -> sql_query($user_application_sql);
			...
			if ($type == 'marketplace') {
				...
				/** Get members of current matchmaker **/
				$superAdminData = $this -> session -> userdata('superadmin_logged_in');
				$matchmakerId = $superAdminData['matchmaker_id'];
				$SQL = 'SELECT user_id,first_name,last_name,gender_id FROM user WHERE matchmaker_id = ' . $matchmakerId . ' ORDER BY first_name ASC,last_name ASC';
				$thisMatchmakerUsers = $this -> general -> sql_query($SQL);
				$data['thisMatchmakerUsers'] = $thisMatchmakerUsers;
	 ...
	 * Inserts data in matchmaker_request_b2b
	 * @author :: Hannan Munshi
	 * **/
	public function process_date_request() {
		...
		if (!empty($post)) {
			...
			$this -> general -> set_table('matchmaker_request_b2b');
	...
	private function loadRequestsData() {
		...
		//Get all the users of this matchmaker and index that array
		$SQL = 'SELECT user_id FROM user ';
		if($superAdminData && isset($superAdminData['matchmaker_id']))
		{
			$matchmakerId = $superAdminData['matchmaker_id'];
			$SQL .= ' WHERE matchmaker_id =' . $matchmakerId;
		}
		
		$res = $this -> general -> sql_query($SQL);
		$thisMatchmakerUsers = array();
		foreach ($res as $key => $value) {
			$thisMatchmakerUsers[$value['user_id']] = $value['user_id'];
		}

		$SQL = 'SELECT b2b.*, sent_user.credits_value, sent_user.first_name as sent_user,sent_matchmaker.company_name as sent_company_name,
			received_user.first_name as received_user,received_matchmaker.company_name as received_company_name
               
                    FROM `matchmaker_request_b2b` as b2b
                    
                    JOIN user as sent_user ON sent_user.user_id = b2b.requested_user_id
                    JOIN user as received_user ON received_user.user_id = b2b.requested_match_user_id
		    JOIN matchmaker as sent_matchmaker on sent_user.matchmaker_id = sent_matchmaker.matchmaker_id
		    JOIN matchmaker as received_matchmaker on received_user.matchmaker_id = received_matchmaker.matchmaker_id';
		    
	      if($matchmakerId != 0)
		  {
		  	         $SQL .= ' WHERE (received_user.matchmaker_id =' . $matchmakerId . ' 
	                          OR sent_user.matchmaker_id =' . $matchmakerId . ')';
			...
			if($matchmakerId != 0)
				...
				case 2 : {//Sent
			      ...
			      JOIN matchmaker as received_matchmaker on received_user.matchmaker_id = received_matchmaker.matchmaker_id';
                    ...
                    if($matchmakerId != 0)
		  			{
                    	$SQL .= ' WHERE (sent_user.matchmaker_id =' . $matchmakerId . ')';
                    }
					break;
				}
				case 3 : {//Accepted
		...
		$SQL .= ' LIMIT ' . ($page_no - 1) * PER_PAGE_ADMIN . ', ' . PER_PAGE_ADMIN;
		...
		foreach ($res as $key => $value) {
			$value['type'] = 'sent';
			if (array_key_exists($value['requested_match_user_id'], $thisMatchmakerUsers) === TRUE) {
				$value['type'] = 'received';
			}

			$SQL = 'SELECT * FROM user_photo WHERE set_primary=1 AND (user_id = ? OR user_id = ?)';
			$primary_photos = $this -> general -> rawQuery($SQL, array($value['requested_user_id'], $value['requested_user_id']));
			foreach ($primary_photos as $ke => $val) {
				...
				} elseif ($value['requested_user_id'] == $val['user_id']) {
					$value['received_user_photo'] = base_url() . 'user_photos/user_' . $val['user_id'] . '/' . $val['photo'];
				}
			}
			$b2bData[$value['matchmaker_request_b2b_id']] = $value;
	...
	public function processRequestsAction() {
		...
		if (!empty($b2bID) && array_key_exists($action, $possibleActions) !== FALSE) {//data is what we expected
			...
			$this -> general -> set_table('matchmaker_request_b2b');
			$updateStastus = $this -> general -> update($dataToUpdate, array('matchmaker_request_b2b_id' => $b2bID));
			$updateStastus = TRUE;
			if ($updateStastus) {
			...
			//$this->session->unset_userdata('sign_up_id');
                        if($limitedAccess === FALSE){
                           ...
                           $this -> session -> set_userdata('ad_id', $user['ad_id']); 
                            ...
                            $this->session->set_userdata('requestedBymm',TRUE);
                        }			
	...
	private function _intro_with_michael($user_id = "") {
        ...
        public function processHandpickMatch(){
           if($this->input->is_ajax_request()){
              ...
              $SQL = 'SELECT user_intro_id FROM user_intro WHERE (user1_id=? OR user1_id = ?) AND (user2_id=? OR user2_id = ?) AND handpicked_time IS NOT null';
              ...
              if(!empty($res)){
                 $response['actionStatus'] = 'warning';
                 $response['msg'] = $usersNames[$handpickedUserID]['first_name'].translate_phrase(' is already in ').$usersNames[$userID]['first_name'].'\'s '.translate_phrase('intro list');
                 die(json_encode($response));
              }
              ...
              $dataToInsert = [
                  ...
                  'intro_availble_time' => $createdTime,
