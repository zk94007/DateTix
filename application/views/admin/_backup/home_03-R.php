...
class Home extends MY_Controller {
	...
	public function shortcut_url($id = 1) {
		...
		if ($url_segment == "join") {			
			...
			if($event_url_datas)
			{
				$event_url_datas = $event_url_datas['0'];
				
				$event_id = $event_url_datas['event_id'];
				$ad_id = $event_url_datas['ad_id'];
				$language_id  = $event_url_datas['display_language_id'];
				...
				if (!empty($event_data)) {
					$redirect_link .= url_city_name($event_data['0']['city_name']) . '/event.html?id=' . $event_id . '&src=' . $ad_id.'&url=' . $url_segment;
				}
			}
			else {
				
				$this -> general -> set_table('event');
				$fields = array('e.event_id,ct.description as city_name', 'ct.city_id');
				$from = 'event as e';
				$joins = array(
						'venue as v' => array('e.venue_id = v.venue_id', 'left'), 
						'neighborhood as n' => array('v.neighborhood_id = n.neighborhood_id', 'left'), 
				...
				unset($where);
				$where['e.shortcut_url'] = $url_segment;
				$where['ct.display_language_id'] = $language_id;
				$where['n.display_language_id'] = $language_id;
				$event_data = $this -> model_user -> multijoins($fields, $from, $joins, $where, '', 'e.event_start_time asc');
				...
				if (!empty($event_data)) {
					...
					//$this -> session -> set_userdata('sess_language_id',$event_data['0']['display_language_id']);
