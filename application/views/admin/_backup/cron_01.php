...
class Cron extends CI_Controller {
	...
	public function fetch() {
		...
		$users = $this -> general -> get("*",array('user_id'=>'1'),array('event_user_id'=>'desc'));
		echo $this->db->last_query();
		echo "<pre>";
		print_r($users);
		exit ;
	...
	public function execute_query() {
		...
		$SQL = 'SELECT user_id,first_name FROM user where user_id = ? OR user_id = ?';
                $usersNames = $this->general->rawQuery($SQL,[2345,2345]);
                echo '<pre>';      
                //echo $this->db->last_query();
                print_r($usersNames);
                die();
                
//                $SQL = 'SELECT status.account_status_id, status.description, last_display_language_id,
//						usr.user_id,completed_application_step,facebook_id,gender_id,first_name,last_name,num_date_tix,attractiveness_level,facebook_page,usr.intro_fee,curr.description as currency, 
//						CASE 
//							WHEN
//								birth_date != \'0000-00-00\'
//								THEN 
//									TIMESTAMPDIFF(YEAR,`birth_date`,CURDATE())
//							END as age FROM user as usr
//                                                   JOIN account_status as status on usr.account_status_id = status.account_status_id                                                   
//                                                   JOIN currency as curr ON curr.currency_id = usr.intro_fee_currency_id WHERE status.display_language_id = 1 AND usr.approved_date IS NOT NULL AND `website_id` !=1 AND `website_id` > 0 GROUP BY usr.user_id ORDER BY `age` DESC  LIMIT 0, 50';
		$query = $this -> db -> query($SQL)->result_array();	
                echo '<pre>';print_r($query);die();
                $sql = "select * from user where website_id = 2";
	...
	public function rsvp_event_reminder()
	{
		...
		$sql_query = "
					SELECT e.event_id, e.event_name, e.event_start_time,rsvp.*, v.*, 
					...
					AND e.display_language_id = '".$language_id."'
		...
		if($event_tickets)
		{
			foreach($event_tickets as $ticket_info)
			{
				...
				if($this -> datetix -> mail_to_user_debug($ticket_info['invite_email_address'], $subject, $email_template))
				{
					$this -> general -> set_table('event_ticket');
					$event_users = $this -> general -> update(array('last_reminder_sent'=>SQL_DATETIME),array('event_ticket_id'=>$ticket_info['event_ticket_id']));	
				}
			}
		}
		else
	...
	public function user_intros($event_id = "") {
		...
		/*
		 ...
		 1) has signed in at least once in past 2 months,
		...
		$usr_condition = 'last_active_time >= now()-interval 2 month AND approved_date IS NOT NULL AND completed_application_step>=6 AND is_contact=1';
	...
	function send_intro_mail() {
		...
		$email_where = "DATE(intro_available_time) <= CURDATE() and intro_email_sent_time < '2000-01-01'";
		$this -> general -> set_table('user_intro');
		$intros_data = $this -> general -> custom_get("*", $email_where);
