...
class Signup extends MY_Controller {
	...
	public function step1()
	{
		...
		if ($postData = $this -> input -> post()) {
			...
			$update_data['current_city_id']   = $this->city_id;
	...
	public function get_city_by_country() {
		...
		echo form_dt_dropdown('current_city_id',$city,'','class="dropdown-dt scemdowndomain" ', translate_phrase('Select city'), "hiddenfield");
	}
	/**
	 * Step 2: 
	 * @access public 
	...
	public function step2()
	{
		...
		if ($postData = $this -> input -> post()) {
			...
			$update_data['religious_belief_id']   = $postData['district'];
	...
	public function step7()
	{
		...
		if ($postData = $this -> input -> post()) {
			...
			$update_data['completed_application_step'] = "7";
			...
			if ($user_friends_with_datetix) {
				foreach ($user_friends_with_datetix as $friend) {
					...
					$subject = translate_phrase('One of your Facebook friends has just applied to DateTix');
					if($user_email_data = $this -> model_user -> get_user_email($friend['user_id']))
					{
						$data['email_content'] = translate_phrase('One of your Facebook friends just applied to DateTix. You now have ') . count($mutual_friend_on_datetix) . ' ' . translate_phrase('Facebook friends on DateTix who can introduce you to their friends for free!') . '</p>
						<p class="lead">' . translate_phrase('Invite more of your friends to apply to DateTix to expand your own dating pool!');
