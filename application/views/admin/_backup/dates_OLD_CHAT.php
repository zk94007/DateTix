...
class Dates extends MY_Controller {
	...
	public function get_more_dates($last_date) {
		...
		while ($current <= $last) {
			...
			$dates[$i]['value'] = date('l, M jS ', strtotime(date('Y-m-d', $current)));
	...
	public function sendChatMessage() {
		...
		if ($diffMinutes > 30) {
			$this -> general -> set_table('user');
			$user_data = $this -> general -> get("", array('user_id' => $currentid));
			$user = $user_data['0'];
	...
	private function sendEmailTousers($from_user_id, $date_id) {
		if ($from_user_id && $date_id) {
			...
			if($is_user_invited)
			{
				...
				$dateDetail = $this -> model_date -> get_date_detail_by_id($date_id);
				$subject = $applicant_user_data['first_name'].translate_phrase(" has applied to your date");
			...
			$date_user_id = $this -> utility -> encode($from_user_id);
			$current_user = $this -> utility -> encode($this -> user_id);
			$data['btn_link'] = base_url() . "dates/chat_history/" . $date_user_id . "/" . $current_user;
