...
class User extends MY_Controller {
	...
	*/
	 ...
	 * - When a user is viewing profile of another user whose matchmaker_id > 0, then don't show chat box and instead show "Date Me" button
		...
		2. Insert row into ""matchmaker_request" table
	 * 
	 * @access public
	 * @param User Id
	 * @return View data
	 ...
	 */
	 public function date_me()
	 {
		...
		if(!$cur_user_id )
		{
			echo 'Please try again with login'; 
			exit;
		}
		...
		if($matchmaker_member_user_id )
		{
			...
			if($matchmaker_id)
			{
				...
				if($matchmaker_datas = $this->general->get("",array('matchmaker_id'=>$matchmaker_id)))
				{
					...
					//save data
					$matchmaker_request['user_id'] = $cur_user_id;
					$matchmaker_request['matchmaker_id'] = $matchmaker_id;
					$matchmaker_request['request_time'] = SQL_DATETIME;	
					$this->general->set_table('matchmaker_request');
					
					$user_datas = $this->general->save($matchmaker_request);
					
					
					$response['type'] = 'success';
					
					$msg = $matchmaker_member_datas['0']['first_name']." is a member of ".$matchmaker_datas['0']['company_name'].". They will contact you soon to arrange a date for you with ".$matchmaker_member_datas['0']['first_name'];
					
					
				}
				else {
					$msg = "Sorry user doens't assosicate with matchmaker";
				}
			}
			else {
					$msg = "Sorry user doens't assosicate with matchmaker";
		...
		$response['msg'] = translate_phrase($msg);
