...
//							else
	...
	public function success_payment() {
		...
		if ($post_data = $this -> session -> userdata('post_data')) {
			...
			$is_successful_order = 0;
			if($is_free_rspv)
			{
				$is_successful_order = 1;
				//10 options to show for invite friends.. Virtually he purchase 10 tickets
				$user_order['num_tickets'] = 5;
			}
			else
			{
		...
		if($postData = $this->input->post())
		{
			if($postData['attendee_emails'])
			{
				...
				foreach($postData['attendee_emails'] as $ticket_id=>$invite_email)
				{
					...
					$check_event_user = $this -> general -> get('*',$check_event_user_condition);
					if(empty($check_event_user))
					{
						$this -> general -> save($event_user_data);
					}
