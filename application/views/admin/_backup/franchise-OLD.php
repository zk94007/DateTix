...
class franchise extends CI_Controller {
	...
	public function __construct() {
		...
		if ($logged_in != TRUE) {
			$user = $this -> config -> item('franchise_username');
			$password = $this -> config -> item('franchise_password');

			if (!isset($_SERVER['PHP_AUTH_USER'])) {
				header('WWW-Authenticate: Basic realm="Datetix Franchise"');
				header('HTTP/1.0 401 Unauthorized');
				echo("Please enter a valid username and password");
				exit();
			} else if (($_SERVER['PHP_AUTH_USER'] == $user) && ($_SERVER['PHP_AUTH_PW'] == $password)) {
				$this -> session -> set_userdata(array('franchise_logged_in' => TRUE));
			} else {
				header('WWW-Authenticate: Basic realm="Datetix Franchise"');
				header('HTTP/1.0 401 Unauthorized');
				echo("Please enter a valid username and password");
