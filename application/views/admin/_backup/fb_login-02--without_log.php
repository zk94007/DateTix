...
class Fb_login extends CI_Controller{
	...
	public function facebook()
	{
		...
		if($userId == 0)
		{
			...
			$data['url'] = $this->facebook->getLoginUrl($access);
			//redirect($data['url']);
		}
		else {
			...
			if ($user  == false) {
				...
				$albumprofile = '';
				$albumsfb = $this->facebook->api('/me/albums?fields=id,type');
				...
				{
					$photoprofilefb = $this->facebook->api('/'.$albumprofile.'/photos?limit='.FB_PHOTO_LIMIT);
	...
	public function bkp_profile_photos()
	{
		...
		$photoprofilefb = $this->facebook->api('/'.$albumprofile.'/photos');
	...
	public function fb_success()
	{
		...
		if (($user = $this->model_user->getByFacebookId($userId)) == false) {
			$step = 1;
		...
		else
		{
			...
			//$url = base_url().url_city_name().'/signup-step-'.$step.'.html';
		}		
	}
	...
	public function import_data($redirect_page = 'edit-profile')
	{
		...
		if($userId == 0)
		{
			//user_religion_politics (remove as per client)
			$access['scope'] = 'email,user_photos,user_activities,user_about_me,user_birthday,user_education_history,user_groups,user_hometown,user_interests,user_likes,user_location,user_relationships,user_relationship_details,user_website,user_work_history,friends_about_me,friends_activities,friends_birthday,friends_education_history,friends_groups,friends_hometown,friends_interests,friends_likes,friends_relationships,friends_relationship_details,friends_religion_politics,friends_website,friends_work_history,friends_location';
			$access['redirect_uri'] = base_url().'fb_login/import_data/'.$redirect_page;
			$data['url'] = $this->facebook->getLoginUrl($access);
			redirect($data['url']);
		}
		else {
			...
			$fb_user = $this->facebook->api('/me');			
			if ($this->model_user->getByFacebookId($fb_user['id']) == false) {
				...
				{
					$user_data['relationship_status_id'] = $gender_id;
				}
