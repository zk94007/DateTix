	...
	*/
	...
	public function user_info($view_user_id="", $login_user_id = '', $pass = '') {
		if(!$view_user_id)
		{
			...
			$user_intros = array();
			if ($login_user_id != '' && is_numeric($login_user_id)) {
			...
			if (!$user_id && !$cur_user_id && !$super_admin) {
				...
				if ($tmp_usr_data) {
					...
					if(isset($super_admin['website_id']) && $super_admin['website_id'])
					{
						...
						if ($cur_user_id) {
							if($view_user_id && $login_user_id)
							{
								$this -> load -> model('model_date');
								$data['date_interection_data'] = $this -> model_date -> get_interaction_detail($view_user_id, $login_user_id);
							}
