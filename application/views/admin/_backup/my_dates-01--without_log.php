		...
		*/
		...
		if ($intros_data) {
			foreach ($intros_data as $intro) {
				...
				//-------------------------------------------------------------------------------------------//
				//Current Lived in
				$fields = array('ct.description as city_description', 'prvnce.description as province_description',  'cntry.country_id','cntry.description as country_description', 'cntry.country_code', 'cntry.flag_url', 'crncy.currency_id', 'crncy.description as currency_description', );
				$from = 'city as ct';
				...
				if ($temp) {
					...
					$country_id = $tmp_intro['intro_current_location']['country_id'];
					unset($temp);
				}
				unset($where);
				...
				$useMeters = $this -> general -> getSingleValue('country', 'use_meters', array('country_id' => $country_id, 'display_language_id' => $this -> language_id));
				$intro_useMeters = $useMeters ? $useMeters : '';
				$feetFrom = '';
				$inchFrom = '';
				
				if ($intro_useMeters == 0 && $intro_useMeters !== FALSE) {
					$cmFrom = $intro['height'];
					if (!empty($cmFrom)) {
						$convertedValues = $this ->datetix->convertToFeet($cmFrom, '', TRUE);
						$tmp_intro['intro_height'] = $convertedValues['feetFrom']."' ".$convertedValues['inchFrom'];
					}
				}
				else {
					$tmp_intro['intro_height'] = $intro['height'].' cm';
				}
				/*-------------------------------------------------------*/
