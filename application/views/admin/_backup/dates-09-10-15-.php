...
class Dates extends MY_Controller {
			...
			if ($decision == '1') {
				...
				if ($update_data['num_date_tix'] >= 0) {
					if ($this -> general -> update($update_data, array('user_id' => $this -> user_id))) {
						...
						// save date ticket log
						$insertLogArray['transaction_time'] = SQL_DATETIME;
						$insertLogArray['description'] = "Apply to Date {$dateid}";
						$insertLogArray['user_id'] = $this -> user_id;
	...
	private function sendEmailTousers($from_user_id, $date_id) {
		if ($from_user_id && $date_id) {
			...
			$this -> general -> set_table('user');
			$request_user_data = $this -> general -> get("first_name,last_name,gender_id", array('user_id' => $from_user_id));
			...
			if($is_user_invited)
			{
				...
				$email_content = $applicant_user_data['first_name'].translate_phrase(' has applied to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']) . ' at ' . date('h:ia', strtotime($dateDetail['date_time'])) . ' on ' . date('d M, Y', strtotime($dateDetail['date_time'])) . '. Click the button below to view '.$noun.' profile and chat with '.$pro_noun.':');
			}	
