...
class My_intros extends MY_Controller {
	...
	public function index($intro_type = 'active') {
		...
		elseif($intro_type == 'upcoming')
		{
			...
			$query = $common_sql . 'AND DATE(intro_expiry_time) >= DATE(CURDATE()) AND DATE(intro_available_time) <= DATE(CURDATE()) group by user_intro.user_intro_id ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC' . $limit;
