...
class Cron extends CI_Controller {
	...
	public function all_users($limit=1) {
		$this -> general -> set_table('user');
		$users = $this -> general -> get("*",array(),array(),$limit);
		echo "<pre>";
		print_r($users);
		exit ;
	}
	...
	public function execute_query() {
		...
		$sql = "ALTER TABLE  `user` ADD `last_intro_mail_sent` DATETIME NULL";
		if ($sql) {
					...
					if ($max_score) {
						...
						if($intros_data = $this -> general -> get("user_id", array('user_id' => $intro_id)))
						{
							...
							else
							{
								...
								$set_intros_data['intro_expiry_time'] = calculate_current_time($user_city -> time_zone_val, "+60 day");
