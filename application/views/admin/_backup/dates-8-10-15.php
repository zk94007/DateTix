...
class Dates extends MY_Controller {
		...
		if ($post = $this -> input -> post()) {
			...
			if ($decision == '1') {
				...
				if ($update_data['num_date_tix'] >= 0) {
					...
					$msg = translate_phrase('You have successfully applied to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']) . ' on ' . date('D, jS M, Y @ h:i A', strtotime($dateDetail['date_time'])) . ' with ') . $dateDetail['first_name'];
	...
	public function my_dates($type="upcoming") {
		...
		if($this->input->is_ajax_request())
		{
			...
			$from = 'date as d';
			$joins = array(
					'date_applicant as da' => array('d.date_id = da.date_id', 'left'), 
	...
	public function updateDateApplicant() {
		if($date_applicant_id = $this -> input -> post('id'))
		{
			...
			if($data['date_info'] = $this -> general -> multijoins_arr($fields, $from, $joins, $where))
			{
				...
				$applicant_data = $this -> general -> get("first_name", array('user_id' => $applicant_user_id));
				...
				$email_data['btn_text'] = translate_phrase('View Match Date');
				$email_data['btn_link'] = base_url() . 'dates/match_date/' . $date_applicant_id;
		...
		if ($this -> input -> is_ajax_request()) {
			...
			if($this -> model_date -> save_chat($insertArray))
			{
				...
				if ($diffMinutes > 30) {
					...
					$data_applicant['email_content'] = 'You have received a new message from ' . $user['first_name'] . ' on ' . date('l ,d M Y', strtotime(date('Y-m-d H:i:s'))) . ' ' . translate_phrase('at') . ' ' . date('h:i A', strtotime(date('Y-m-d H:i:s')));
					$data_applicant['email_title'] = '';
					$data_applicant['btn_link'] = base_url() . 'dates/chat_history/' . $this -> utility -> encode($currentid) . '/' . $this -> utility -> encode($otherid);
	...
	private function sendEmailTousers($from_user_id, $date_id) {
		if ($from_user_id && $date_id) {
			...
			$this -> general -> set_table('user');
			$user_data = $this -> general -> get("first_name,last_name", array('user_id' => $this -> user_id));
			$applicant_user_data = $user_data['0'];

			$this -> general -> set_table('user');
			$request_user_data = $this -> general -> get("first_name,last_name", array('user_id' => $from_user_id));
			...
			if($is_user_invited)
			{
				...
				$email_content = $applicant_user_data['first_name'].translate_phrase(' has applied to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']) . ' at ' . date('h:ia', strtotime($dateDetail['date_time'])) . ' on ' . date('d M, Y', strtotime($dateDetail['date_time'])) . '. Click the button below to view his profile and chat with him:');
			}			
			...
			$data['btn_link'] = base_url() . "dates/chat_history/" . $chat_with_user_id. "/" . $host_user_id;
	...
	public function match_date($id) {
		...
		$custom_where = '(requested_user_id = "'.  $this -> user_id.'" OR applicant_user_id = "'.  $this -> user_id.'")';
		...
		if($data['date_info'] = $this -> general -> multijoins_arr($fields, $from, $joins, $where,$custom_where))
		{
			...
			if($host_user_id == $this->user_id)
			{
				...
				$data['host_user_data'] = $data['user_data'];
			}
			else
			{
				$data['applicant_user_data'] = $data['user_data'];
				$this -> general -> set_table('user');
				$host_data = $this -> general -> get("user_id,first_name,last_name,num_date_tix,mobile_phone_number", array('user_id' => $host_user_id));
				$data['host_user_data'] = $host_data['0'];
