...
class Dates extends MY_Controller {
	...
	public function test()
	{
		...
		//echo "<pre>";print_r($user_dates  );
		
		
		//$this->general->set_table('date');
		//$user_dates  = $this->general->get("*",array('requested_user_id'=>3511),array(),10);		
		//echo $this->db->last_query();
		//echo "<pre>";print_r($user_dates  );		
		
		$this->general->set_table('log_user_date_ticket');
		$user_dates  = $this->general->get("*",array(),array('log_user_date_ticket_id'=>'desc'),40);		
		echo $this->db->last_query();
	...
	public function new_date_step4() {
		...
		if (!empty($post)) {
			...
			$invited_users = array();
			if(isset($date_packages[$post['date_package_id']]))
			{
				...
				$people = $this->model_people->get_prefered_matche_profiles($condition_str,0,$date_packages[$post['date_package_id']]['profile_matches_count'],'user.user_id');				
				if($people)
				{
					$dateDetail = $this -> model_date->get_detail_by_id($date_id);
					if($dateDetail)
					{
						foreach($people as $match_user)
						{
							if($dateDetail)
							{
								if($user_name = $this->model_date->send_date_invitation($match_user['user_id'],$dateDetail,1))
								{
			...
			$this->general->set_table('user');
            $user_account_info = $this->general->get("num_date_tix",array('user_id'=>$this -> user_id));
	...
	public function new_date_step6() {
		...
		if ($this -> input -> post()) {
			...
			if ($user_email_data) {
				...
				$data['email_title'] = translate_phrase('Hi '.$user['first_name'].', You have confirmed following date, Please check below details');
				$data['email_content']  = '<h3>'.translate_phrase('Date Confirmation').'</h3> 	
	                                <p><strong>'.print_date_daytime($dateDetail['date_time']).' '.$dateDetail['name'].", ".$dateDetail['address'].'</strong></p>
	                                <p>'.translate_phrase('Looking For').' <strong>'.$dateDetail['gender'].'</strong> '.translate_phrase('For').' <strong>'.$dateDetail['intention_type'].'</strong></p>';
					
				$data['email_content']  .= 
					'<table cellspacing="3" style="width:100%" align="left">';
					
					
		        	if(strtotime($dateDetail['date_time']) <= strtotime('+1 day'))
		        	{
		        		$data['email_content'] .= '<tr>
		                    <th>'.translate_phrase("Last minute date").': </th>
		                    <td>'.LAST_MINUTE_DATE.translate_phrase("date tickets").'</td>
		                </tr>';
		        	}
					...
					{
						$data['email_content'] .= '<tr>
		                    <th>'.$dateDetail["intention_type"].': </th>
		                    <td>'.$dateDetail["relationship_num_date_tix"].' '.translate_phrase("date tickets").'</td>
		                </tr>';
					}
		        	
					$data['email_content'] .= '<tr>
		                    <th>'.$dateDetail["venue"].' '.translate_phrase("Budget").': </th>
		                    <td>'.$dateDetail["budget_num_date_tix"].' '.translate_phrase("date tickets").'</td>
		                </tr>';
							
			       	$date_packages = $data['date_packages'];	        
			        if($dateDetail['date_package_id'] && isset($date_packages[$dateDetail['date_package_id']])){
			        	$data['email_content'] .= '<tr>
			                <th>'.$date_packages[$dateDetail["date_package_id"]]["name"].' '.translate_phrase("Date Package").': </th>
			                <td>'.$date_packages[$dateDetail["date_package_id"]]["use_tickets"].' '.translate_phrase("date tickets").'</td>
			            </tr>';	
			        }
				...
				$data['email_content'] .= '
	                <tr>
	                    <th>'.translate_phrase('Total').': </th>
	                    <td>'.$total.' '.translate_phrase('date tickets').' </td>
	                </tr>
	                <tr>
	                	<th colspan="2" style="color:darkgreen">'.translate_phrase('You will receive 100% refund of your date tickets if you receive less than 3 applications').'</th>
	                </tr>
	            </table>';
	...
	public function user_chats() {
		...
		{
			$getChatDetail = $this -> model_date -> get_chat_history($user_id,$page_no);
			...
			if ($dateApplicants = $this -> general -> get("applicant_user_id", array('date_id' => $date_id,'status'=>1))) {
				...
				foreach ($dateApplicants as $applicant) {
					...
					if ($user_email_data) {
						...
						$this -> general -> set_table('user');
						$user_data = $this -> general -> get("user_id,password", array('user_id' => $this -> user_id));
						$user = $user_data['0'];
	...
	public function view_date($date_id) {
		...
		if($data['date_info']['requested_user_id'] != $this -> user_id)
		{
			...
			$this -> general -> set_table('date_applicant');
			if ($info = $this -> general -> get("count(date_applicant_id) as total_applications", array('date_id' => $data['date_info']['date_id']))) {
