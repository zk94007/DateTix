...
//							else
			...
			if ($response -> status() == Merchant_response::COMPLETE) {
				...
				if ($event_order_id = $this -> general -> simple_save($user_order)) {
					...
					if($user_mail)
					{
						...
						$email_data['email_content'] .= "<br/><br/>".translate_phrase('The event starts at ').date("g:i a",strtotime($event_data['event_start_time'])).translate_phrase(" on ").date(DATE_FORMATE,strtotime($event_data['event_start_time'])).translate_phrase(' and takes place at').':';
	...
	private function _get_ticket_package($event_data) {
		...
		if ($ticket_packages) {
			...
			if ($this -> session -> userdata('signup_discount') == 'yes' && $event_data['max_prepaid_tickets'] > SMALL_EVENT_USER_LIMIT) {
				...
				if ($event_data['ad_id'] != 4)
				{
					$discount = 0.632;
				}
				elseif ($event_data['ad_id'] == 4)
				{
					$discount = 0.632;
				}
			...
			if ($this -> session -> userdata('fb_like_discount') == 'yes') {
	...
	*/
	...
	public function event_list()
	{
		...
		$data['past_event_data'] = $this -> model_user -> multijoins($fields, $from, $joins, $where,'','e.event_start_time asc');
