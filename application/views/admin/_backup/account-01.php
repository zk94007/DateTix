...
class Account extends CI_Controller {
	...
	public function settings() {
		...
		$data['requested_photo_view_users'] = array();
		if($user['photo_requested_by_ids'])
		{
			$data['requested_photo_view_users'] = $this->general->sql_query("select user_id, first_name,last_name from user where user_id IN(".$user['photo_requested_by_ids'].")");
		}
		
		$data['approved_photo_view_users'] = array();
		if($user['photo_viewable_by_ids'])
		{
			$data['approved_photo_view_users'] = $this->general->sql_query("select user_id, first_name,last_name from user where user_id IN(".$user['photo_viewable_by_ids'].")");
		}
		
		
		$data['page_title'] = translate_phrase('Your Setting');
		$data['page_name'] = 'user/account/setting';
		//echo "<pre>";print_r($data['requested_photo_view_users']);exit;
		$this -> load -> view('template/editProfileTemplate', $data);
	}
	 ...
	 * @author Rajnish Savaliya
	 */
	public function photo_request($requested_user_id=1) {
		...
		if ($postData = $this -> input -> post()) {
			...
			if($user_photo_privacy_data = $this->general->get("user_id,photo_requested_by_ids,photo_viewable_by_ids",$request_condition))
			{
				if($user_photo_privacy_data['0']['photo_requested_by_ids'])
				{
					$all_requested_ids = explode(',',$user_photo_privacy_data['0']['photo_requested_by_ids']);
					$all_requested_ids = array_diff($all_requested_ids,array($requested_user_id));
				}
				
				$updated_request_data['photo_requested_by_ids'] = implode(',',$all_requested_ids);
				if($this->general->update($updated_request_data,$request_condition))
				{
					$response['type'] = "1";
					if($postData['status'] == "approve")
					{
						if($user_photo_privacy_data['0']['photo_viewable_by_ids'])
						{
							$viewable_ids = explode(',',$user_photo_privacy_data['0']['photo_viewable_by_ids']);
							array_push($viewable_ids,$requested_user_id);
							$viewable_ids = array_unique($viewable_ids);
						}
						else
						{
							$viewable_ids = array($requested_user_id);
						}
						
						$response['data'] = $viewable_ids;
						$updated_approve_data['photo_viewable_by_ids'] = implode(',',$viewable_ids);					
						if($this->general->update($updated_approve_data,$request_condition))
						{
							$response['msg'] = translate_phrase("Photo request has been approved.");
						}
					}
					else
					{
						$response['msg'] = translate_phrase("Photo request has been rejected.");
					}		
				}
			}
			else{
