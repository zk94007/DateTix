...
class Admin extends CI_Controller {
	...
	public function index() {
		...
		$user_application_sql = "SELECT city.description AS city_description, status.account_status_id, status.description,  last_display_language_id,
					user_id,completed_application_step,facebook_id,gender_id,first_name,last_name,num_date_tix,attractiveness_level,facebook_page
					FROM user as usr
                                        JOIN matchmaker on usr.matchmaker_id = matchmaker.matchmaker_id
					JOIN account_status as status on usr.account_status_id = status.account_status_id					
					JOIN city on usr.current_city_id = city.city_id
					WHERE 
	...
	public function load_users($type = 'review_app') {
		...
		if ($this -> input -> is_ajax_request()) {
			...
			$user_application_sql = "SELECT city.description as city_description, status.account_status_id, status.description, last_display_language_id,
						usr.user_id,completed_application_step,facebook_id,usr.gender_id,first_name,last_name,num_date_tix,attractiveness_level,facebook_page,usr.credits_value,
						CASE 
							WHEN
			...
			if ($type == "manage_member" || $type == 'marketplace') {
				...
				if ($type == 'marketplace') {
					...
					{
						$user_application_sql .= " AND `matchmaker_id` != '" . $find_match_of_userdata['matchmaker_id'] . "'";
					}
				}
			}
	...
	private function loadRequestsData() {
		...
		$SQL = 'SELECT user_id FROM user WHERE matchmaker_id > 0';
		if($superAdminData && isset($superAdminData['matchmaker_id']))
		{
			$matchmakerId = $superAdminData['matchmaker_id'];
			$SQL .= ' AND matchmaker_id =' . $matchmakerId;
		}
		...
		$SQL = 'SELECT b2b.*,sent_user.first_name as sent_user,received_user.first_name as received_user
