...
class Home extends MY_Controller {
	...
	public function load_landing_page($short_url = "") {
		...
		$fields = array('page.*,msg.main_message as banner_message');
		$from = 'landing_page as page';
		$joins = array('landing_page_main_message as msg' => array('msg.landing_page_id = page.landing_page_id', 'inner'));		
		$lang_condition['page.shortcut_url'] = $short_url;
		$lang_condition['msg.display_language_id'] = $language_id;
		
		if ($landing_page_data = $this -> model_user -> multijoins($fields, $from, $joins, $lang_condition)) {
				
			$data = $landing_page_data['0'];
			$data['page_title'] = 'Homepage';
			
			
			/* Landing Page data [School and Professional] */
			...
			$this -> load -> view($data['html_url'] . '/index', $data );
		}
	}
	...
	public function shortcut_url($id = 1) {
		...
		$language_id = $this -> session -> userdata('sess_language_id');
		$event_id = $this -> uri -> segment(1);

		if ($event_id == "join") {
			$redirect_link .= url_city_name($event_data['0']['city_name']) . '/apply.html?src=12';
		} else {
			if (!is_numeric($event_id)) {
				if ($event_id == 'tinavip') {
				...
				} elseif ($event_id == 'waleed') {
					$event_id = 5;
					$ad_id = 14;
				} elseif ($event_id == 'novel') {
					$event_id = 5;
					$ad_id = 15;
					...
					$event_id = 5;
					$ad_id = 20;
				} elseif ($event_id == 'halloween') {
					$event_id = 5;
					$ad_id = 1;
				} elseif ($event_id == 'angel') {
					$event_id = 5;
					$ad_id = 24;
				} elseif ($event_id == 'redhills') {
					$event_id = 5;
					$ad_id = 26;
				} else {
			...
			$from = 'event as e';
			$joins = array('venue as v' => array('e.venue_id = v.venue_id', 'inner'), 'neighborhood as n' => array('v.neighborhood_id = n.neighborhood_id', 'inner'), 'city as ct' => array('ct.city_id = n.city_id', 'inner'));

			$where['e.event_id'] = $event_id;
			//$where['ct.display_language_id'] = $language_id;
			//$where['n.display_language_id'] = $language_id;
			//$where['e.display_language_id'] = $language_id;
			if ($event_data = $this -> model_user -> multijoins($fields, $from, $joins, $where, '', 'e.event_start_time asc')) {
				$redirect_link .= url_city_name($event_data['0']['city_name']) . '/event.html?id=' . $event_id . '&src=' . $ad_id;
			}
		}
		...
		if ($this -> form_validation -> run() == false) {
			...
			if ($exist) {
				...
				if ($completed_step != "") {
					...
					$insert_id = $this -> model_user -> insert_user(array('first_name' => $this -> input -> post('first_name'), 'last_name' => $this -> input -> post('last_name'), 'ref_user_id' => $invite_id, 'password' => sha1($this -> input -> post('password'))));
