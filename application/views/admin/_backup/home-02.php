...
class Home extends MY_Controller {
	...
	public function load_landing_page($short_url = "") {
		...
		if($language_id!="")
		{
			...
			$landing_page_condition .= " AND display_language_id=" . $language_id;	
		}
		...
		{
			if($website_data = $query->result_array())
			{
				$website_id = $website_data['0']['website_id']; 
				$landing_page_condition .=  " AND website_id REGEXP '[[:<:]]".$website_id."[[:>:]]'";			// [Website landing page - off]
			}

		}
		
		//echo "<pre>";print_r($landing_page_condition);exit;
		if ($landing_page_data = $this -> general -> get("", $landing_page_condition)) {
			...
			$this -> load -> view($landing_page_data['html_url'] . '/index.html');
		}
		else {
