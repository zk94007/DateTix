	...
	$.ajax({ 
		...
		success: function (response) {
			...
			if(user_id && is_like)
			{
				loading();
				$.ajax({ 
					url: '<?php echo base_url(); ?>' +"account/apply_fb_discount_on_tickets/"+is_like, 
					type:"post",
					 dataType:'json',
					success: function (response) {
						stop_loading();
...
$package_base_price = $this->user_id?$event_info['price_online_discounted']: $event_info['price_online'];
				...
				<div class="emp-B-tabing-M-short" id="events_tabs">
					...
					<div class="step-form-Part">
						...
						<div class="getBox" align="left">
							...
							<?php else:?>
							<img style="max-height:98%; max-width:100%;" src="<?php echo $event_info['flyer_url'];?>" />
							<?php endif;?>
						</div>
						<!-- END Event Photo Section -->
												
						<?php if($event_info['description']!=""):?>
							<div class="getBox"><p><?php echo $event_info['description'];?></p></div>
						<?php endif;?>
									...
									<form id="payment_form"
										...
										<?php endif;?>
											...
											<input type="hidden" name="url" id="url"
												value="<?php echo isset($event_info['url'])&& $event_info['url']?$event_info['url']:'';?>" />
										...
										<?php if($event_info['max_prepaid_tickets'] > SMALL_EVENT_USER_LIMIT):?>
											<!-- Member Discount block start -->	
											<?php  if($package_base_price - $event_info['partner_discount'] <= 0):?>
												<?php if($signup_discount):?>
													<div class="style01 comn-top-mar"><span class="DarkGreen-color"><b><?php echo 'You may attend the event for free because you are a DateTix member!';?></b></span></div>
												<?php else:?>
													<div class="style01 comn-top-mar"><span class="Red-color"><b><a href="<?php echo base_url().url_city_name();?>/apply.html?highlight=1"><?php echo translate_phrase("Attend for free")?></a><?php echo translate_phrase(" this event by ")?><a href="<?php echo base_url().url_city_name();?>/apply.html?highlight=1"><?php echo translate_phrase("applying")?></a><?php echo translate_phrase(" or ")?><a href="<?php echo base_url().url_city_name();?>/signin.html"><?php echo translate_phrase("signing in")?></a><?php echo translate_phrase(" to your free ").get_assets('name','DateTix').translate_phrase(" account")?>!</b></span></div>
												<?php endif;?>
											<?php else:?>
