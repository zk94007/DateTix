...
<?php $is_box_show = 0;?>
	...
	<?php echo translate_phrase('Enjoy 30% off all date ticket packages because you have the ')?>
		<a class="blu-color"
			href="<?php echo base_url() . url_city_name() ?>/upgrade-account.html"><?php echo translate_phrase("Discounted Date Tickets account upgrade")?></a>!
	</div>
...
<?php if($intro['user_id'] != 1 || $user_data['gender_id'] == 2):?>
<div
	class="Half-datebox"
	...
	<?php if($intro_not_interested == '0000-00-00 00:00:00'):?>
		...
		<?php if(1==2 && isset($tab) && $tab == 'expired_intro' && !$is_premius_member_re_intros):?>
		...
		<?php else:?>
			...
			<?php if($not_interested == '0000-00-00 00:00:00'):?>
			<div class="dateBoxHed">
			<?php echo translate_phrase('Want to meet').' '.$intro['intro_name']?>? <?php echo translate_phrase('Click the button below and one of our friendly dating consultants will help arrange a date for you!');?>
			</div>
			<?php endif;?>
			<!-- Ticket Management -->
			<?php //include APPPATH.'views/user/include/ticket_mgt.php';?>
			...
			<div class="appear-prt-but">
				<div class="suggest-btn">
					<?php if($user_info['num_date_tix'] > 0 || 1==1):?>					                                             
                                                <a href="javascript:;" onclick="UserDateInsert('<?php echo $this->utility->encode($intro['user_intro_id'])?>','<?php echo $intro['user_id'];?>')">
						<!--<a href="javascript:;" id="date_me" onclick="dateMe('<?php echo $user_info['user_id'];?>',this)">-->
                                        <?php else:?>
						<a href="<?php echo base_url().url_city_name()?>/get-more-tickets.html">
                                        <?php endif;?>
					
					<!--<a href="<?php echo base_url().url_city_name().'/suggest-date-idea.html?return_to='.$return_page.'&intro='.$this->utility->encode($intro['user_intro_id'])?>">-->
						<?php echo translate_phrase('Date ') . $intro['intro_name'];?> </a>
				</div>
				...
				<!--<div
					...
					<?php endif;?>
                            ...
                            <div id="dateSaveMessage_<?php echo $this->utility->encode($intro['user_intro_id'])?>">
                                <h4>                     
                                    <span id="DarkGreen-color" color="DarkGreen-color" style="color:green"></span>                                                                        
                                    <span id="Red-color" color="Red-color"></span>                                                                        
                                </h4>
                            </div>
			</div>
		<?php endif;?>
