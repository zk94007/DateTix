												...
												<div class="skil-check-area-01">
													<ul>
														<li><span> <input type="checkbox" checked="checked"
																...
																<label class="choice" for="show_company_name"><?php echo translate_phrase('Show company name on your profile')?>
															</label> </span>
														</li>
													</ul>
												</div>
...
function send_verification_mail(user_school_id,mail_for,email){
 ...
 function career_validaion()
 {
     var flag = true;
     if(jQuery('#career_stage_id').val() == '')
     {
         jQuery('#careerStageError').text('<?php echo translate_phrase("Please specify your career stage");?>');
         flag = false;
     }
     else
     {
         jQuery('#careerStageError').text('');
     }
			...
			success: function (data) {
				...
				if(data!="0")
				{
					...
					prevInsertCompany = company_name;
                                            ...
                                            $.ajax({ 
                                                    url: '<?php echo base_url(); ?>' +"user/show_industries/", 
                                                    type:"html",
                                                    data:'',
                                                    cache: false,
                                                    success: function (data1) {
                                                            $('#company_industry').html(data1);
