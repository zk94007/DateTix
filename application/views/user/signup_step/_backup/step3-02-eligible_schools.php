			...
			success: function (data) {
				...
				$( "#school_name" ).autocomplete({
					...
					minLength: 1,
						...
						select : function(){ jQuery('#selectedFromAvailableSchools').val('yes'); jQuery('#school_error').text('');}
				});
			}     
		});
	}
...
<script>
    ...
    function get_school_id()
    {
        ...
        {
         $.ajax({ 
            url: '<?php echo base_url(); ?>' +"user/get_school_id", 
            type:"post",
            dataType : 'json',
            ...
            success: function (response) {
               ...
               {
                   $("#selectedFromAvailableSchools").val("yes");
                   jQuery('#school_error').text('');
                  // $('#user_school_id').val(response.school_id);
               }
               else
               {
                   $("#selectedFromAvailableSchools").val("no");
                   jQuery('#school_error').text("<?php echo translate_phrase('Sorry. This school data is not yet available with us.Please try adding it later')?>.");
                   //$('#user_school_id').val(response.school_id);
               }
                /*if (data.response > 0) {
    ...
    selectedFromAvailableSchools = jQuery('#selectedFromAvailableSchools').val();
        //alert(selectedFromAvailableSchools);
    if(selectedFromAvailableSchools == 'yes'){
	if (document.signup.is_degree_completed.checked) {
      ...
      }//end of if condition for selected from available schools
      else
      {
          if(school_name==""){
              $("#school_error").html('');
            $('#schoo_name_err').addClass('error_indentation error_msg');
          ...
          //if(yr_start != "" && yr_end != "" && yearsAttendedRangeIsCorrect === false)
          if((yr_start != "" || yr_start != 0) && (yr_end != "" || yr_end != 0) && yearsAttendedRangeIsCorrect === false)
            {
                $('#years_attended_error').addClass('error_indentation error_msg')
                $('#years_attended_error').html('<?php echo translate_phrase("End year must be equal to or after start year")?>');
          ...
          //if(yr_start =="" || yr_end == "")  
          if((yr_start =="" || yr_start == 0) || (yr_end == "" || yr_end == 0))  
            {
                $('#years_attended_error').addClass('error_indentation error_msg')
                $('#years_attended_error').html('<?php echo translate_phrase("Please tell us during which years you attended this school")?>');
            }
          ...
          if(school_name != "" && degree !="")
            {
                jQuery('#schoo_name_err').text('');
                jQuery('#school_error').text('<?php echo translate_phrase("Sorry, this school has not yet been added to DateTix\'s eligible schools list. Please check back later"); ?>');
            }
      }
    }
    ...
    function edit_school(school_id){
      ...
      // jQuery('#years_attended_start').val('');
       ...
       jQuery('#selectedFromAvailableSchools').val('yes');
       var ex = jQuery('#selectedFromAvailableSchools').val();
...
// var selectedFromAvailableSchools = jQuery('#selectedFromAvailableSchools').val();
// if(selectedFromAvailableSchools == 'yes')
 //{
	var school_name = $('#school_name').val();
	loading();
