...
var handpicked_page_no = "<?php echo $page_no ?>";
var preventAjaxCall = [];
$(document).ready(function(){
	...
	*/
	...
	//When select a option..
    $("#active_expire ul li.active a").live('click',function () {
    	var currentTab = '';    	
    	...
    	if($(this).attr('href') == '#upcoming')
    	{
    		currentTab = 'upcoming';
    	}
    	
    	if($(this).attr('href') == '#expired')
    	{
    		currentTab = 'expired';
    	}
    	if($(this).attr('href') == '#handpicked')
    	{
    		currentTab = 'handpicked';
    	}
...
var handpickedData = true;
	
function initialize_data(currentTab)
{
    var isFlag = true;
    if(currentTab == 'handpicked'){
        isFlag = handpickedData;
    }
	...
	if(isAjaxCallRunning == false && isFlag)
	{
    	$("#"+currentTab+"_container").html('<div class="div_data_loader"></div>');
    	$.ajax({ 
            url: '<?php echo base_url(); ?>' +"my_intros/index/"+currentTab, 
            type:"post",
            success: function (response) {
            	stop_loading();
	            if(preventAjaxCall.indexOf(currentTab) !=  '-1')
                ...
                if(currentTab == 'handpicked')
                {
                    handpickedData = false;
                    msg = '<p><span class="no-rows"><?php echo translate_phrase('You currently have no handpicked intros.')?></span></p>';
                }
	                        	            	
            	if(currentTab == 'active')
                {
                    activeData = false;
                    msg = '<p><span class="no-rows"><?php echo translate_phrase('You currently have no active intros.')?></span></p>';
                }

                if(currentTab == 'upcoming')
                {
                    upcomingData = false;
                    msg = '<p><span class="no-rows"><?php echo translate_phrase('You currently have no upcoming intros.')?></span></p>';
                }
                if(currentTab == 'expired')
                {
                    expiredData = false;
                    msg = '<p><span class="no-rows"><?php echo translate_phrase('You currently have no expired intros.')?></span></p>';
                }
...
$(window).scroll(function() {  
	...
	if( totalScrollAmount >= $(document).height()) 
	{
		...
		var offset = 1;
                ...
                if($('#active_expire ul li.active a').attr('href') == '#handpicked')
		{
			currentTab = 'handpicked';
			offset += parseInt(handpicked_page_no); 
		}
		if($('#active_expire ul li.active a').attr('href') == '#active')
		{
			currentTab = 'active';
		...
		<div class="Apply-Step1-a-main">
			<div class="emp-B-tabing-prt">
				<div class="emp-B-tabing-M-short" id="active_expire">
					...
					<ul class='etabs'>
						<li class='tab tab-nav'><span></span><a id="ActiveDateTab" href="#active"><?php echo translate_phrase('Active') ?></a></li>
						<li class='tab tab-nav'><span></span><a id="UpcominEasyTab" href="#upcoming"><?php echo translate_phrase('Upcoming') ?> </a></li>
						<li class='tab tab-nav'><span></span><a id="PastDateTab" href="#expired"><?php echo translate_phrase('Expired') ?></a></li>
                       <li class='tab tab-nav'><span></span><a id="HandpickedTab" href="#handpicked"><?php echo translate_phrase('Handpicked') ?></a></li>
					</ul>
					<!-- Active Tab content Start -->
					<div class="step-form-Main Mar-top-none Top-radius-none" id="active">
							...
							<div style="display:<?php echo $is_active_intro?'block':'none;'?>" class="sortby bor-none Mar-top-none srtby-pad">
								...
								<div class="sortbyDown">
									<dl class="sort_dl dropdown-dt domaindropdown">
										<dt>
											<a href="javascript:;" key="1"><span><?php echo translate_phrase('Message Time').' ('.translate_phrase('recent to old').')';?>
											</span> </a> <input type="hidden" name="intro_order"
												id="intro_order" value="1">
										</dt>
										<dd>
											<ul>
												<li><a href="javascript:;" key="1"><?php echo translate_phrase('Message Time').' ('.translate_phrase('recent to old').')';?>
												</a></li>
												<li><a href="javascript:;" key="2"><?php echo translate_phrase('Message Time').' ('.translate_phrase('old to recent').')';?>
												</a></li>

												<li><a href="javascript:;" key="3"><?php echo translate_phrase('Intro Date').' ('.translate_phrase('recent to old').')';?>
												</a></li>
												<li><a href="javascript:;" key="4"><?php echo translate_phrase('Intro Date').' ('.translate_phrase('old to recent').')';?>
												</a></li>

												<li><a href="javascript:;" key="5"><?php echo translate_phrase('Age').' ('.translate_phrase('young to old').')';?>
												</a></li>
												<li><a href="javascript:;" key="6"><?php echo translate_phrase('Age').' ('.translate_phrase('old to young').')';?>
												</a></li>

												<li><a href="javascript:;" key="7"><?php echo translate_phrase('First Name').' ('.translate_phrase('A to Z').')';?>
												</a></li>
												<li><a href="javascript:;" key="8"><?php echo translate_phrase('First Name').' ('.translate_phrase('Z to A').')';?>
												</a></li>
											</ul>
										</dd>
									</dl>
					...
					<div class="step-form-Main Mar-top-none Top-radius-none" id="upcoming">
						<div class="step-form-Part">
							<div id="upcoming_container"><?php //$this->load->view('user/intro/upcoming_intro');?></div>
						</div>
					</div>

					<!-- Expired Tab content Start -->
					<div class="step-form-Main Mar-top-none Top-radius-none" id="expired">
						<div class="step-form-Part">
							<div id="expired_container"><?php //$this->load->view('user/intro/expired_intro');?></div>
						</div>
					</div>
					<!-- Expired Tab content END -->

                                        <!-- Handpicked Tab content Start -->
					<div class="step-form-Main Mar-top-none Top-radius-none" id="handpicked">
						<div class="step-form-Part">
							<div id="handpicked_container"><?php //$this->load->view('user/intro/handpicked_intro');?></div>							
						</div>
					</div>
					<!-- Handpicked Tab Content END -->
				</div>
				<!-- END emp-B-tabbing-M -->
			</div>
		</div>
