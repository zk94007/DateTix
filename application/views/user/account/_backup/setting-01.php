...
$(document).ready(function(){
	...
	<?php if($redirect_tab_name  = $this->session->userdata('redirect_tab_name')):?>
		...
		<?php if(($status = $this->input->get('status')) && ($requested_user_id = $this->input->get('requested_user_id'))):?>
			view_photo_request('<?php echo $status;?>','<?php echo $requested_user_id;?>');
		<?php endif;?>
...
function view_photo_request(status,approve_user_id)
{
	
	//var approve_user_id =  $(obj).parent().attr('lang');
	var url = '<?php echo base_url("account/photo_request")?>';
	if(approve_user_id)
	{
		loading();
		$.ajax({ 
			url:url+"/"+approve_user_id, 
			type:"post",
			data:'status='+status,
			cache: false,
			dataType:'json',
			success: function (data) {
				...
				if(data.type == "1")
				{
					...
					{
						$(".applications").parent().fadeOut(function(){$(this).remove()});
					}
					else
					{
						$('#list_'+approve_user_id).fadeOut(function(){$(this).remove()});
					}
				...
				<div class="emp-B-tabing-prt">
					<div class="emp-B-tabing-M-short" id="active_expire">
						...
						<div class="step-form-Main Mar-top-none Top-radius-none"
							...
							<?php if($requested_photo_view_users):?>
							<div class="step-form-Part sortby">
								...
								<ul class="applications cms-list comn-top-mar"">
									<?php foreach($requested_photo_view_users as $user):?>
									<li class="list" id="list_<?php echo $user['user_id'];?>">
											<span><?php echo $user['first_name'].' '.$user['last_name']?></span>
											<span class="btns">
												<button onclick="view_photo_request('approve','<?php echo $user['user_id'];?>')" type="button" class="btn btn-pink btn-small" lang="<?php echo translate_phrase("Approved");?>"><?php echo translate_phrase("Approve");?></button>
												<button onclick="view_photo_request('reject','<?php echo $user['user_id'];?>')" type="button" class="btn btn-blue btn-small" lang="<?php echo translate_phrase("Rejected");?>"><?php echo translate_phrase("Reject");?></button>
											</span>
									</li>									
									<?php endforeach;?>							
								</ul>
