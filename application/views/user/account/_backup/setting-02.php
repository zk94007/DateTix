			...
			success: function (data) {
				...
				if(data.type == "1")
				{
					if(status == '1'){
						if(oldStatus != "0")
						{
							var tmp = $('#list_'+user_photo_request_id).find('.decline').text();
							$('#list_'+user_photo_request_id).find('.decline').text($('#list_'+user_photo_request_id).find('.decline').attr('lang'))
							$('#list_'+user_photo_request_id).find('.decline').attr('lang',tmp);
						}
						tmp = $('#list_'+user_photo_request_id).find('.approve').text();
						$('#list_'+user_photo_request_id).find('.approve').text($('#list_'+user_photo_request_id).find('.approve').attr('lang'))
						$('#list_'+user_photo_request_id).find('.approve').attr('lang',tmp);	
					}
					else if(status == '2'){
						if(oldStatus != "0")
						{
							var tmp = $('#list_'+user_photo_request_id).find('.approve').text();
							$('#list_'+user_photo_request_id).find('.approve').text($('#list_'+user_photo_request_id).find('.approve').attr('lang'))
							$('#list_'+user_photo_request_id).find('.approve').attr('lang',tmp)
						}
						tmp = $('#list_'+user_photo_request_id).find('.decline').text();
						$('#list_'+user_photo_request_id).find('.decline').text($('#list_'+user_photo_request_id).find('.decline').attr('lang'))
						$('#list_'+user_photo_request_id).find('.decline').attr('lang',tmp);
					...
					<div class="emp-B-tabing-M-short" id="active_expire">
						...
						<div class="step-form-Main Mar-top-none Top-radius-none"
							...
							<div class="step-form-Part sortby">
								<h2><?php echo translate_phrase('Photo Request for view your photos?')?></h2>
								...
								<ul class="applications cms-list comn-top-mar"">
									...
									<li class="list">
										...
										<?php 
											...
											{
												$tmp = $btn_appr_txt;
												$btn_appr_txt = $btn_appr_toggle_txt;
												$btn_appr_toggle_txt = $tmp;
												$tmp ="";
											}
											elseif($user['status'] == '2')
											{
												$tmp = $btn_dcln_txt;
												$btn_dcln_txt = $btn_dcln_toggle_txt;
												$btn_dcln_toggle_txt = $tmp;
												$tmp ="";
											}
										?>
										<span class="btns"  id="list_<?php echo $user['user_photo_request_id'];?>" lang="<?php echo $user['status'];?>">
											<button onclick="view_photo_request('1','<?php echo $user['user_photo_request_id'];?>')" type="button" class="btn btn-pink btn-small approve" lang="<?php echo $btn_appr_toggle_txt;?>"><?php echo $btn_appr_txt;?></button>
											<button onclick="view_photo_request('2','<?php echo $user['user_photo_request_id'];?>')" type="button" class="btn btn-blue btn-small decline" lang="<?php echo $btn_dcln_toggle_txt;?>"><?php echo $btn_dcln_txt;?></button>
										</span>
										<p><?php echo format_chat_time($user['request_time'])?> </p>
									</li>									
									<?php endforeach;?>							
								</ul>
							</div>
