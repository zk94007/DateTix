...
<script type="text/javascript">
	...
	$(document).ready(function(){		
		...
		$('ul.partnerList > li > a').live('click',function(){
			...
			$("#partnerErrorMsg").fadeOut();
			
			row.find('.visitor_cnt').text(visitors);
			
			row.find('.languageDD dd>ul>li>a[key="'+default_display_language_id+'"]').click();
			
		});
	});
	function validate_partner_form(){
		var isValid = true;
		$.each($("#partnerTable tbody tr"),function(i,item){
			if($(item).find('input[lang=partner_id]').val() == "" || $(item).find('input[lang=partner_id]').val() == "0")
			{
				isValid = false;
			}
		});
...
?>
<form onsubmit="return validate_partner_form()" method="post" id="event_partners" name="event_partners" action="<?php echo base_url($this->admin_url.'/save_event_partner')?>">
<input type="hidden" name="event_id" value="<?php echo isset($event_id)?$event_id:'';?>" />	
		...
		<table width="100%" class="centerTable" id="partnerTable">
			<thead>
				<tr>
					...
					<th width="10%"><?php echo translate_phrase('Unique Visitors');?></th>
					<th width="30%"><?php echo translate_phrase('URL Shortcut');?></th>
				...
				<?php if(isset($event_urls) && $event_urls):?>
					<?php foreach($event_urls as $event_url):?>
						<tr class="event_url_row" lang="<?php echo $event_url['event_url_id']?>">
							<td>
								...
								if(isset($partners) && $partners):
									...
									foreach($partners as $partner)
									{
										...
										{
											$select = $partner['name'];
											$unique_visitors = $partner['unique_visitors'];
										}	
									}
								?>
								<?php if(isset($is_past_event)&&$is_past_event):?>
								...
								<dl class="dropdown-dt domaindropdown parterDL">
									<dt>
										...
										<input type="hidden"lang="partner_id" name="event_url[<?php echo $event_url['event_url_id']?>][partner_id]" value="<?php echo $event_url['partner_id']?>" required="">
									</dt>
									<dd>
										<ul style="display: none;" class="partnerList">
											<?php foreach($partners as $partner):?>
											<li><a visitors="<?php echo $partner['unique_visitors']?>" default_display_language_id="<?php echo $partner['default_display_language_id'];?>" default_event_url="<?php echo $partner['default_event_url'];?>"  key="<?php echo $partner['partner_id'];?>"><?php echo $partner['name'];?></a></li>
											<?php endforeach;?>													
										</ul>
									</dd>							
								</dl>
							...
							<td>
								<span class="visitor_cnt"><?php echo $unique_visitors?></span>
							</td>
							<td>
								<?php if(isset($is_past_event)&&$is_past_event):?>
									<?php echo $event_url['url']?>
								<?php else:?>	
								<span class="input-label datetix-url"></span>
	...
	<div class="userTopRowHed">
		...
		&nbsp; &nbsp;  &nbsp; &nbsp;  
		<a href="<?php
		 echo base_url($this->admin_url.'/add_new_partner'.$event_id.'?city_id='.$selected_city_id);?>"><?php echo translate_phrase('Create Partner')?></a>	
		 
	</div>
	<div class="userTopRowHed">
		<br/>
		<p class="Red-color" id="partnerErrorMsg" style="display:none;"><?php echo translate_phrase('You must select a partner for each URL shortcut');?></p>
	</div>
...
<tr class="hidden event_url_row" id="dummyRowPartner" lang="0">
	...
	<td><span class="visitor_cnt"></span></td>
	<td>
		<span class="input-label datetix-url"></span>
