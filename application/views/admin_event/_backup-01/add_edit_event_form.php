...
Date.prototype.addHours= function(h){
	...
	$(document).ready(function(){
		...
		/*$('#add_edit_event').validate({  
				...
				rules: {
		            'description': {
		                required: function(){CKEDITOR.instances.description.updateElement();}
		            }
		        },		        
			});*/
...
<div class="Indicate-top"> *&nbsp; <?php echo translate_phrase('Indicates required field')?></div>
	...
	<div class="sfp-1-main">
		<div class="sfp-1-Left bold"><?php echo translate_phrase('Display Language')?>: <span>*</span></div>
		<div class="sfp-1-Right">		
			<?php
			$language_id = isset($display_language_id)?$display_language_id:$this->language_id;
			if(isset($is_past_event)&&$is_past_event):?>
				<?php 
					if($languages){
						foreach($languages as $lang_id=>$lang)
						{
							...
							{
								echo '<input type="hidden" name="display_language_id" value="'.$language_id.'"/>
								<label class="input-alt-lbl">'.$lang.'</label>';	
							}
						}
					}
				?>
			<?php else:?>
			<?php echo form_dt_dropdown('display_language_id',$languages,$language_id,'class="dropdown-dt domaindropdown languageDD"',translate_phrase('Select Language'),"hiddenfield"); ?>
			<?php endif;?>		
		</div>
	</div>
	...
	<div class="userBox-wrap comn-top-mar">
		<table width="100%" class="centerTable" id="listTable">
			<thead>
				<tr>
					<th width="10%"><?php echo translate_phrase('Display Language')?>: <span class="pink-colr">*</span></th>
					<th width="60%"><?php echo translate_phrase('Event Description');?>: <span class="pink-colr">*</span></th>
