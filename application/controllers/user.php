	...
	*/
	public function load_notification() {
		...
		if($type=='all')
		{
			...
			$data['unread_msg_notification'] = $this->model_date->total_unread_msg($user_id);
			
			// * Upcoming dates query **/
			$fields = array('d.date_id as date_id');		                       
			$from = 'date as d';
			$joins = array(
			...
			$custom_where = '(da.applicant_user_id = "' . $user_id . '" OR d.requested_user_id = "' . $user_id. '")';		
			$order_by = 'd.date_time asc';
			$this -> db -> group_by('d.date_id');
			$data['active_dates_notification'] = $user_dates  = $this -> general -> multijoins_arr($fields, $from, $joins, $where,$custom_where,$order_by,null,null,'count');
		}
				
		if($type=='all'||$type=='find_date_cnt')
		{
			$data['find_date_count'] = $this->model_date->get_other_date_detail($user_id, NULL, NULL,'count');	
		}
		
		if($type=='all'||$type=='find_people_cnt')
		{
			$this->load->model('model_people');
			$condition_str = $this->model_people->formate_prefered_match_cluase($this->model_people->get_user_preference($user_id));
			$people = $this->model_people->get_prefered_matche_profiles($condition_str, NULL,NULL,'user.user_id');
			//echo $this->db->last_query();exit;
			$data['find_people_count'] = count($people);
		}
	...
	public function confirmation() {
		...
		$user_data = $this -> general -> get("first_name,from_url", array('user_id' => $user_id));
		
		if($event_ticket_id = $this->session->userdata('event_ticket_id'))
		{
			$fields = array('rsvp.*','e.*','el.event_name','v.*','n.description as neighborhood_name','ct.description as city_name');
			...
			if($data['event_info'] = $this -> model_user -> multijoins($fields, $from, $joins, $where))
			{
				...
				if($is_updated)
				{
					...
					if($user_email_data = $this -> model_user -> get_user_email($user_id))
					{
						...
						$this -> model_user -> send_email(INFO_EMAIL,$user_email_data['email_address'], $rsvp_subject, $email_template,"html",get_assets('name','DateTix'));
					}
				}				
		...
		$data['page_name'] = 'user/confirmation';
                $data['from_url']=$user_data['0']['from_url'];
	...
	public function edit_profile() {
		...
		$data['city_id'] = $city ? $city -> city_id : '';
		$data['city'] = $this -> model_user -> get_city($language_id, $country_id);
		...
		if($data['has_district'] = $this -> model_user -> check_district_exist($language_id, $city_id))
		{
		...
		$data['country'] = $this -> model_user -> get_country_city($this -> language_id);
	...
	/**
	 * Step 1: Ajax call for get district[Neighbourhood] by city
	 * @access public 
	...
	public function get_district_by_city()
	{
		...
		{
			if($district  = $this -> model_user -> get_district($this -> language_id, $city_id))
			{
				echo '<div class="sfp-1-main">
					<div class="sfp-1-Left">'.translate_phrase('Neighborhood').':<span>*</span></div>
					<div class="sfp-1-Right">
						'.form_dt_dropdown('district', $district, '', 'class="dropdown-dt reqdropdown"', translate_phrase('Select neighborhood'), "hiddenfield").'
						<label id="districtError" class="input-hint error" error_txt="'.translate_phrase("Please select neighborhood").'"></label>
					</div>
				</div>';	
			}
		}
	...
	public function ideal_match() {
		...
		if ($this -> input -> is_ajax_request()) {
			...
			unset($where);
			$where['child.user_id'] = $user_id;
			$where['parent.language_id'] = $language_id;;
			//
	...
	/**
	 * - When a user is viewing profile of another user whose website_id > 0, then don't show chat box and instead show "Date Me" button
		- When the "Date Me" button is clicked, do the following:
		1. Send email to "website.contact_email_address" with the email subject "Free consultation request" and email body "Michael Ye wants to date with your member Jennifer Chan. You may contact him at 6684-2770 and mikeye27@gmail.com to follow-up."
		2. Insert row into ""date_request" table
	 * 
	 * @access public
	 * @param User Id
	 * @return View data
	 ...
	 */
	 public function date_me()
	 {
		...
		if(!$cur_user_id )
		{
	 	...
	 	$website_member_user_id = $this->input->post('user_id');
		$response['type'] = 'error';
		$msg = '';
		
		if($website_member_user_id )
		{
			
			$this->general->set_table('user');
			$website_id = 0;
			
			if($website_member_datas = $this->general->get("first_name,last_name,website_id",array('user_id'=>$website_member_user_id)))
			{
				$website_id = $website_member_datas['0']['website_id'];
			}
			else {
			...
			if($website_id)
			{
				...
				if($website_datas = $this->general->get("",array('website_id'=>$website_id)))
				{
					$website_email = $website_datas['0']['contact_email_address'];
					
					
					$this->general->set_table('user');
					$user_datas = $this->general->get("first_name,last_name,gender_id,mobile_phone_number",array('user_id'=>$cur_user_id));
					
					$user_email_data = $this -> model_user -> get_user_email($cur_user_id);
																																																																																																																								
					// Email Datas
					$data['email_content'] = '';
						
					$subject = translate_phrase('Free consultation request');
					$pro_noun = ($user_datas['0']['gender_id'] == 1) ? translate_phrase('him') : translate_phrase('her');
				
					$data['email_title'] = $user_datas['0']['first_name'].' '.$user_datas['0']['last_name'].translate_phrase(' wants to date with your member ').$website_member_datas['0']['first_name'].' '.$website_member_datas['0']['last_name'].translate_phrase('. You may contact ').$pro_noun.translate_phrase(' at ').$user_datas['0']['mobile_phone_number'].translate_phrase(' and ').$user_email_data['email_address'].translate_phrase(' to follow-up.');

					$email_template = $this -> load -> view('email/common', $data, true);
					//echo $email_template;exit;
					$this -> datetix -> mail_to_user($website_email, $subject, $email_template);
					
					//save data
					$date_request['user_id'] = $cur_user_id;
					$date_request['requested_user_id'] = $website_member_user_id;
					$date_request['request_time'] = SQL_DATETIME;	
					$this->general->set_table('date_request');
					
					$user_datas = $this->general->save($date_request);																														
					$response['type'] = 'success';
					
					//$msg = $website_member_datas['0']['first_name'].translate_phrase(" is a member of ").$website_datas['0']['company_name'].translate_phrase(". They will contact you soon to arrange a date for you with ").$website_member_datas['0']['first_name'];
					$msg = translate_phrase("One of our matchmakers will contact you soon to try to arrange a date with ") . $website_member_datas['0']['first_name'];
				}
				else {
					//$msg = translate_phrase("Sorry, we are unable to fulfill your request as this user hasn't yet been assigned to any matchmaker yet.");
					$msg = translate_phrase("Sorry, we are unable to fulfill your request as this user doesn't belong to any website in our network.");
				}
			}
			else {
					//$msg = translate_phrase("Sorry, we are unable to fulfill your request as this user hasn't yet been assigned to any matchmaker yet.");
        ...
        /**
         * function to insert into user_date table if user clicked Date Michael and showed interest in having consultation session
	- When the "Date Me" button is clicked, do the following:
	1. Send email to "website.contact_email_address" with the email subject "Free consultation request" and email body "Michael Ye wants to date with your member Jennifer Chan. You may contact him at 6684-2770 and mikeye27@gmail.com to follow-up."
	2. Insert row into ""user_date" table
         * @access public
         * @author Jigar Oza
         * @return success message
         */
        
        public function insert_user_date(){
            if($this->input->is_ajax_request()){
		...
		if(!$cur_user_id )
		{
	 	...
	 	$website_member_user_id = $post['intro_user_id'];
		...
		if($website_member_user_id )
		{
			...
			if($website_id)
			{
				...
				if($website_datas = $this->general->get("",array('website_id'=>$website_id)))
				{
					...
					//save data					
			                $dataToInsert=array(
			                    'user_intro_id'=>$this->utility->decode($post['intro_id']),
			                    'date_suggested_by_user_id'=>$cur_user_id,
			                    'date_suggested_time'=>SQL_DATETIME
			                );
								                
			                $this->general->set_table('user_date');
			                $userDateID=$this->general->save($dataToInsert);
			                if($userDateID){
					    echo translate_phrase("Your date request has been received. One of our consultants will contact you soon to help you arrange a date with ") . $website_member_datas['0']['first_name'].'!';
			                }else{
			                    echo  translate_phrase("An error has occurred. Please contact info@datetix.com");
			                }
				}
				else {
					echo translate_phrase("Sorry, we are unable to fulfill your request as this user doesn't belong to any website in our network.");
				}
			}
			else {
					echo translate_phrase("Sorry, we are unable to fulfill your request as this user doesn't belong to any website in our network.");
	...
	public function user_info($view_user_id="", $login_user_id = '', $pass = '') {
		if(!$view_user_id)
		{
			...
			if(!$login_user_id && $logged_in_user_id)
			{
				$decode_logged_user_id = $logged_in_user_id;
			}
			...
			/* HIDE INTRO START [CHECK - HIDE INTRO END BELOW] */
			
			if (!$user_id && !$cur_user_id && !$super_admin) {
				if ($cur_user_id) {
				...
				if ($user_intros) {
					...
					if (!$user_id || !$decode_logged_user_id) {
					...
					//* COMMENT ON OLD DATES DATA */
					$date_data = array();//$this -> general -> sql_query($query);
				...
				if ($tmp_usr_data) {
					...
					if ($tmp_usr_data['facebook_id'] && $user_datas['facebook_id'] && $cur_user_id != $user_id) {
					...
					if(isset($super_admin['website_id']) && $super_admin['website_id'])
					{
						...
						if ($cur_user_id) {
							...
							if($user_id>0 && $cur_user_id >0 && $user_id != $cur_user_id)
							{
								$encoded_other_user_id = $this -> utility -> encode($user_id);
								$encoded_current_user_id = $this -> utility -> encode($cur_user_id);
								$this -> load -> model('model_date');
								$data['date_interection_data'] = $this -> model_date -> get_interaction_detail($encoded_other_user_id, $encoded_current_user_id);
								//echo $this->db->last_query();exit;
							}
					...
					{
						// checl following iteraction
                    	$data['checkFollowing']=$this->db->query("select * from user_follow_user where
                            user_id='".$this->utility->decode($login_user_id)."'
                            and follow_user_id='".$this->utility->decode($view_user_id)."' and 
	...
	public function invite_friends() {
		...
		$fields = array('ct.description as city_name','ct.city_id','e.*','el.event_name');
		$from = 'event as e';
		$joins = array(
					'event_language as el' => array('el.event_id = e.event_id', 'left'), 							
					'venue as v' => array('e.venue_id = v.venue_id', 'inner'), 
	...
	public function sign_in_using_email($return_url = '') {
		...
		if ($this -> input -> post()) {
			...
			if ($emailExists === TRUE){
				...
				if($approved_date == "" && 1==2)
				{
					$response = array('success' => 0, 'message' => translate_phrase('Sorry, Your account is not approved.'));
					die(json_encode($response));
				}
				else if($facebookId != "")
				{
					...
					if (!empty($password) && sha1($password) == $userPassword) {
						...
						$this -> session -> set_userdata('sess_city_id', $userDetails->current_city_id);
	...
	public function delete_profile_photo() {
		...
		echo $this -> model_user -> delete_profile_photo($photo_id, $user_id);
	}
	...
	public function delete_user_by_email() {
        ...
        public function show_industries(){
                ...
                if($result->num_rows()>0){
                        ...
                        echo form_dt_dropdown('industry_id',$industry,'','class="dropdown-dt majordowndomain"',translate_phrase(' Select company industry '),"hiddenfield");				
                }else{
                        echo "0";
                }
        }
        ...
        public function follow_user(){
            $post=$this->input->post();
            $follow_user_id=$post['id'];
            $status=$post['type'];
            
            if($status=='0'){
                $this->db->where('follow_user_id ',$follow_user_id);
                $this->db->where('user_id',$this -> user_id);
                $this->db->update('user_follow_user', array('unfollow_time'=>SQL_DATETIME));
                echo 'Successfully Unfollowed.!';
            }else{
                // checl aready any entry is there or not
                $this->db->where('follow_user_id ',$follow_user_id);
                $this->db->where('user_id',$this -> user_id);
                $updateFollowingtime=$this->db->update('user_follow_user', array('follow_time'=>SQL_DATETIME));
                if($this->db->affected_rows() == 0){
                    $insertArray['user_id']=$this -> user_id;
                    $insertArray['follow_user_id']=$follow_user_id;
                    $insertArray['follow_time']=SQL_DATETIME;
                    $saveFollowUser=$this->model_user->saveUserFollowing($insertArray);
                }
