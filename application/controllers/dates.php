...
class Dates extends MY_Controller {
	...
	public function new_date_step1() {
		...
		if (!empty($post))
		{
			...
			if($this -> user_id != 0)
			{
				$sql = "SELECT * FROM date WHERE completed_step >='".REQUIRED_DATE_COMPLETED_STEP."' AND requested_user_id = '".$this -> user_id."' AND `date_time` between '".date('Y-m-d H:i:s',strtotime($insertArray['date_time']))."'-INTERVAL 2 HOUR AND '".date('Y-m-d H:i:s',strtotime($insertArray['date_time']))."'+INTERVAL 2 HOUR ";
		...
		if(!$this->input->get('want_date_with'))
		{
			$this->session->unset_userdata('date_with_user');			
		}
	...
	public function new_date_step2() {
		...
		if (!empty($post)) {
			...
			$updateArray['date_relationship_type_id'] = $post['looking_for'];
	...
	public function new_date_step3() {
		...
		$data['last_neighborhood_id'] = "";
		
		if($data['last_host_date'] = $this -> model_date -> last_date_preference($this -> user_id))
		{
			$this->general->set_table('merchant');
			if($tmp_data = $this->general->get('neighborhood_id',array('merchant_id'=>$data['last_host_date']['merchant_id'])))
			{
				$data['last_neighborhood_id'] = $tmp_data['0']['neighborhood_id'];	
			}
		}
	...
	public function new_date_step4() {
		...
		//$this -> session -> set_userdata('save_date_id',86);
		
		$date_packages = $this->_get_date_packages();
		$date_id = $this -> session -> userdata('save_date_id');
		...
		$this -> general -> set_table('user');
		$user_data = $this -> general -> get("user_id,first_name,last_name,password,num_date_tix,mobile_phone_number ,facebook_id,want_age_range_lower,want_age_range_upper,gender_id,ethnicity_id", array('user_id' => $this -> user_id));
		$user = $user_data['0'];
		
		$post = $this -> input -> post();
		if (!empty($post)) {
			
			$want_date_with = $this->session->userdata('date_with_user');
			$is_more_invited = isset($post['more_invited'])?$post['more_invited']:1;
			
			$date_id = $this -> session -> userdata('save_date_id');
			
			$updateArray['num_date_tickets'] = $post['date_tickets'];
			$updateArray['date_gender_ids'] = $post['gender'];
			$updateArray['age_range_lower'] = $post['start_age'];
			$updateArray['age_range_upper'] = $post['end_age'];
			...
			if(isset($date_packages[$post['date_package_id']]) && $is_more_invited != 0)
			{
				//Find Prefered Matches
				//$user_info = $this->model_people->get_user_preference($this->user_id);
				$user_info['want_age_range_lower'] = $post['start_age'];
				$user_info['want_age_range_upper'] = $post['end_age'];
				
				if(isset($post['gender']) && $post['gender'])
				{
					$arr = explode(',', $post['gender']);
					foreach($arr as $value)
					{
						if($value)
							$user_info['user_want_gender'][]['gender_id'] = $value;	
					}
				}
				
				if(isset($post['ethnicity']) && $post['ethnicity'])
				{
					$arr = explode(',', $post['ethnicity']);
					foreach($arr as $value)
					{
						if($value)
							$user_info['user_want_ethnicity'][]['ethnicity_id'] = $value;	
					}
				}
				
				$condition_str = $this->model_people->formate_prefered_match_cluase($user_info);		
				$people = $this->model_people->get_prefered_matche_profiles($condition_str,0,$date_packages[$post['date_package_id']]['profile_matches_count'],'user.user_id','count');
				$tic += $date_packages[$post['date_package_id']]['use_tickets'];		
			}

			if($want_date_with)
			{
				if($dateDetail && $want_date_with['user_id'])
				{
					//Date Invited
					if($user_name = $this->model_date->send_date_invitation($want_date_with['user_id'],$dateDetail,1,'new_dates'))
					{
						$invited_users[] = $user_name;									
					}
															
					//Date Accepted
					
					$insertArrayDecision['user_id'] = $this -> user_id;
					$insertArrayDecision['target_user_id'] = $want_date_with['user_id'];
					$this -> general -> set_table('user_decision');					
					if($this -> general -> checkDuplicate($insertArrayDecision))
					{
						$update_data['decision'] = 1;
						$update_data['decision_time'] = SQL_DATETIME;
						$this -> general -> update($update_data, $insertArrayDecision);
					}
					else {						
						$insertArrayDecision['decision_time'] = SQL_DATETIME;
						$insertArrayDecision['decision'] = 1;						
						$this -> general -> simple_save($insertArrayDecision);
					}
					unset($insertArrayDecision);
					
					//$this->general->set_table('date_invite');
					//$this->general->update(array('status'=>1),array('date_id'=>$date_id,'invite_user_id'=>$want_date_with['user_id']));
				}
			}
			...
			if($people)
			{
				if($dateDetail)
				{
					foreach($people as $match_user)
					{
						if($user_name = $this->model_date->send_date_invitation($match_user['user_id'],$dateDetail,1,'new_dates'))
						{
							$invited_users[] = $user_name;									
						}
					}
			...
			if($invited_users)
			{
				//$last_user = end($invited_users);
				if(count($invited_users) > 5)
				{
					$top_five = array();
					for($i=0;$i<5;$i++)
					{
						$top_five[] = $invited_users[$i];
					}
					
					$last_user = count($invited_users) - 5;
					$invited_users_str = implode(', ', $top_five).' and '.$last_user.' others' ;
				}
				else {
					if(count($invited_users) == 1)
					{
						$invited_users_str = implode(', ', $invited_users);	
					}
					else
					{
						$last_user = array_pop($invited_users);
						$invited_users_str = implode(', ', $invited_users).' and '.$last_user ;
					}
				}
			}
			
			$invite_msg = translate_phrase('You have successfully invited ' . $invited_users_str. ' to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']). ' on ').print_date_daytime($dateDetail['date_time']);
			$this->session->set_flashdata('package_invitation_msgs',$invite_msg);
			
			$this->general->set_table('user');
            $user_account_info = $this->general->get("first_name,num_date_tix",array('user_id'=>$this -> user_id));
			if($user_account_info)
			{
				$available_tickets = $user_account_info['0']['num_date_tix'];
				if($available_tickets > $tic)
				{
					//substract from  user
					$this -> db -> query("update user set num_date_tix=num_date_tix-$tic where user_id='" . $this -> user_id . "'");
				}
				else {
					$this->session->set_flashdata('active_package_id',$updateArray['date_package_id']);
					redirect(base_url() . "dates/new_date_step4");
				}
			}
			...
			if ($user_email_data) {
				...
				$data['email_title'] = "";
				$data['email_content']  = '<h3>'.translate_phrase('Hi '.$user['first_name'].', you have posted the following date:').'</b>';
				$data['email_content']  .= '<h4>'.translate_phrase('Date Confirmation').'</h4> 	
	                                <p><strong>'.print_date_daytime($dateDetail['date_time']).' '.$dateDetail['name'].", ".$dateDetail['address'].'</strong></p>
	                                
	                                <p>';	                                
	          if($dateDetail['gender'])
	          	$data['email_content']  .= translate_phrase('Looking For').' <strong>'.$dateDetail['gender'].'</strong> ';	                                
			  
			  if($dateDetail['intention_type'])
	          	$data['email_content']  .= translate_phrase('For').' <strong>'.$dateDetail['intention_type'].'</strong>';
			  	                                
	          $data['email_content']  .= '</p>';					
				$data['email_content']  .= 
					'<table cellspacing="3" width="450">';
					
					
		        	if(strtotime($dateDetail['date_time']) <= strtotime('+1 day'))
		        	{
		        		$data['email_content'] .= '<tr>
		                    <th align="left" >'.translate_phrase("Last minute date").': </th>
		                    <td align="left">'.LAST_MINUTE_DATE.translate_phrase("date tickets").'</td>
		                </tr>';
		        	}
					...
					{
						$data['email_content'] .= '<tr>
		                    <th align="left">'.$dateDetail["intention_type"].': </th>
		                    <td align="left">'.$dateDetail["relationship_num_date_tix"].' '.translate_phrase("date tickets").'</td>
		                </tr>';
					}
		        	
					$data['email_content'] .= '<tr>
		                    <th align="left">'.$dateDetail["venue"].' '.translate_phrase("Budget").': </th>
		                    <td align="left">'.$dateDetail["budget_num_date_tix"].' '.translate_phrase("date tickets").'</td>
		                </tr>';
							
			        if($dateDetail['date_package_id'] && isset($date_packages[$dateDetail['date_package_id']])){
			        	$data['email_content'] .= '<tr>
			                <th align="left">'.$date_packages[$dateDetail["date_package_id"]]["name"].' '.translate_phrase("Date Package").': </th>
			                <td align="left">'.$date_packages[$dateDetail["date_package_id"]]["use_tickets"].' '.translate_phrase("date tickets").'</td>
			            </tr>';	
			        }
		        
		    	
				
				$total=LAST_MINUTE_DATE+$dateDetail['relationship_num_date_tix']+$dateDetail['budget_num_date_tix'];
	            if($dateDetail['date_package_id'] && isset($date_packages[$dateDetail['date_package_id']]))
				{
					$total += $date_packages[$dateDetail['date_package_id']]['use_tickets'];
				}
				$data['email_content'] .= '
	                <tr>
	                    <th align="left">'.translate_phrase('Total').': </th>
	                    <td align="left">'.$total.' '.translate_phrase('date tickets').' </td>
	                </tr>
	            </table>
				<p style="color:darkgreen">'.translate_phrase('You will receive 100% refund of your date tickets if you receive less than 3 applications').'</p>';
	            
		                    
				$return_url = base_url() . 'dates/view_date/' . $date_id;
				...
				$data['btn_text'] = 'View Date';
				
				
				$email_template = $this -> load -> view('email/common', $data, true);
				$subject = translate_phrase("Date Posting Confirmation");
		...
		$data['gender_want']['age_range'] = @$lastDateDetail['age_range_lower'].'-'.@$lastDateDetail['age_range_upper'];
		...
		$data['date_packages'] = $date_packages;
	...
	public function new_date_step6() {
		...
		$this -> general -> set_table('user');
		$user_data = $this -> general -> get("user_id,first_name,last_name,num_date_tix,mobile_phone_number ,password, facebook_id,want_age_range_lower,want_age_range_upper,gender_id,ethnicity_id", array('user_id' => $this -> user_id));
		$user = $user_data['0'];
		
		$date_id = $this -> session -> userdata('save_date_id');		
		$dateDetail = $this -> model_date -> get_date_detail_by_id($date_id, $this -> language_id);
		$data['date_packages'] = $this->_get_date_packages();
		$data['date_detail'] = $dateDetail;
	...
	private function _get_date_packages()
	{
		return array(
			'1'=>array(
				'name'=> 'Bronze',
	 ...
	 */
	public function find_date_list($dateid = '') {
		$this->_isLoggedIn();
		$this -> session -> unset_userdata('save_date_id');
		if ($this -> session -> userdata('return_url'))
			$this -> session -> unset_userdata('return_url');
		...
		if($this->input->is_ajax_request())
		{
			...
			$data['other_dates'] = $this -> model_date -> get_other_date_detail($this -> user_id, $limit,$offset);
			$this -> load -> view('user/dates_app/include/load_find_dates', $data);
		}
		else {
		
			$data['page_name'] = 'user/dates_app/find_date_list';
			$this -> load -> view('template/editProfileTemplate', $data);	
		}
	}
	...
	public function find_dates($dateid = '') {
		...
		if (!empty($dateid)) {
			...
			if (!$checkUserInvited) {
				...
				if($result = $this -> model_date -> get_date_detail_by_id($dateid))
				{
					if($result['requested_user_id'] == $this->user_id)
					{
						redirect('dates/view_date/'.$dateid);
					}
					...
					if($merchant_photo = $this -> db -> get_where('merchant_photo', array('merchant_id' => $result['merchant_id'])) -> row_array())
					{
						$results[$key] = array_merge($results[$key],$merchant_photo );
					}
					
					$data['other_dates'] = $results;
				}
				else {
		...
		if ($post = $this -> input -> post()) {
			...
			if ($decision > 0) {
				...
				if ($update_data['num_date_tix'] >= 0) {
					...
					$superLbl = "";
					if($this->input->post('num_date_tickets') > 49)
					{
						$superLbl = "super ";
					}
					$msg = translate_phrase('You have successfully '.$superLbl.'applied to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']) . ' on ' . print_date_daytime($dateDetail['date_time']) . ' with ') . $dateDetail['first_name'];
				...
				if(isset($post['target_user_id']) && $post['target_user_id'])
				{
					$insertUserDecision['user_id'] = $this -> user_id;
					$insertUserDecision['decision'] = $decision;
					$insertUserDecision['decision_time'] = SQL_DATETIME;
					$insertUserDecision['target_user_id'] = $post['target_user_id'];
					
					$this -> general -> set_table('user_decision');
					$this -> general -> save($insertUserDecision);
				}
	...
	public function apply_for_date() {
		...
		if ($date_id) {
			...
			$msg = translate_phrase('You have successfully applied to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']) . ' on ' . print_date_daytime($dateDetail['date_time']) . ' with ') . $dateDetail['first_name'].translate_phrase(" now to get to know him better");
	...
	public function test_user_chat()
	{
		$this->_isLoggedIn();
		...
		$data['user_data'] = $user;
		//$getChatDetail = $this -> model_date -> get_chat_history_pagination($this -> user_id,$page_no);
		//echo "<pre>";print_r($getChatDetail);exit;
		...
		if($this->input->is_ajax_request())
		{
			...
			$this -> load -> view('user/dates_app/include/load_user_chats', $data);	
		}
		else {
			
			$data['page_title'] = translate_phrase('User Chat History');
			$data['page_name'] = 'user/dates_app/user_chats_TEST';
	...
	public function user_chats() {
		...
		{
			//PAGINATION REMOVED..
			$getChatDetail = $this -> model_date -> get_chat_history($this -> user_id,$page_no);
			$data['chat_history'] = $getChatDetail;
			$this -> load -> view('user/dates_app/include/load_user_chats', $data);	
		}
		else {
	...
	public function chat_history($other_user = '', $current_user = '') {
		...
		if (!empty($other_user) && !empty($current_user)) {
			...
			$is_read_condition['to_user_id'] = $this -> utility -> decode($current_user);
			$is_read_condition['from_user_id']= $this -> utility -> decode($other_user);;
			$is_read_condition['is_read'] = '0';
			$this->general->set_table('user_chat');
			$this->general->update(array('is_read'=>1),$is_read_condition);
	...
	public function get_looking_message() {
		...
		$cond['display_language_id'] = $this->language_id;
		$cond['relationship_type_id'] = $id ;
		
		$this->general->set_table("relationship_type");
		$message = "";
		if($getPoints = $this -> general -> get("",$cond))
		{
			$message = $getPoints['0']['description'] .' '.translate_phrase("will cost you") .' '.$getPoints['0']['num_date_tix'] .' '.translate_phrase("date tickets");
		}
	...
	public function my_dates($type="upcoming") {
		...
		if($this->input->is_ajax_request())
		{
			...
			$fields = array('d.date_id as date_id, d.requested_user_id as requested_user_id, d.date_time as date_time, d.status as status,
			...
			$joins = array(
					...
					'gender as g' => array('FIND_IN_SET(g.gender_id,d.date_gender_ids)', 'left')
	...
	public function cancel_date() {
		...
		if ($isUpdated) {
			...
			if ($dateApplicants = $this -> general -> get("applicant_user_id", array('date_id' => $date_id,'status'=>1))) {
				$subject = $date_type . " @ " . $date_address." " . translate_phrase("has been cancelled by ").$date_info['first_name'];
				foreach ($dateApplicants as $applicant) {
					...
					if ($user_email_data) {
						$data['email_content'] = $date_type . ' @ ' . $date_address . 'on ' . $date_time . translate_phrase(' that you applied to has been by ').$date_info['first_name'];
						$data['email_title'] = '';
						
						//Dynamic autologin link
						/*
						$this -> general -> set_table('user');
						$user_data = $this -> general -> get("user_id,password", array('user_id' => $user_id));
						$user = $user_data['0'];
	...
	public function withdraw_applicant() {
		...
		if ($isUpdated) {
			...
			$date_info = $this -> model_date -> get_date_detail_by_id($date_id);
			$user_id = $date_info['requested_user_id'];
			$date_type = ($date_info['date_type'])?$date_info['date_type']:'date';
			$date_address = $date_info['name'];
			$date_time = print_date_daytime($date_info['date_time']);
			if($user_email_data = $this -> model_user -> get_user_email($user_id))
			{
				$this -> general -> set_table('user');
				$user_data = $this -> general -> get("user_id,first_name,last_name,gender_id", array('user_id' => $this->user_id));
				$user = $user_data['0'];
				$gender_pn = $user['gender_id']==1?'him':'her';
				$gender_pn_1 = $user['gender_id']==1?'his':'her';
				
				$subject = $user['first_name'].' '.translate_phrase('has withdrawn '.$gender_pn_1.' application to your date');
				$data['email_content'] = $user['first_name'].' '.translate_phrase('has withdraw his application from '.$date_type). ' @ ' . $date_address . ' at ' . $date_time;
				$data['email_title'] = '';
		...
		if($date_applicant_id = $this -> input -> post('id'))
		{
			...
			if($data['date_info'] = $this -> general -> multijoins_arr($fields, $from, $joins, $where))
			{
				...
				if($applicant_user_email = $this -> general -> get("email_address", array('user_id' => $applicant_user_id)))
				{
					...
					$email_data['email_content'] = $user['first_name'] . translate_phrase(' has chosen you to be ') . $gender_type . translate_phrase(' date for ') . $date_type . ' @ ' . $date_address . ' on ' . $date_time;
	...
	private function sendEmailTousers($from_user_id, $date_id) {
		if ($from_user_id && $date_id) {
			...
			*/
			...
			$subject = $applicant_user_data['first_name'].translate_phrase(" has ".$superLbl."applied to be your date");
			...
			$email_content = $applicant_user_data['first_name'].translate_phrase(' has '.$superLbl.' applied to ' . $dateDetail['date_type'] . ' @ ' . trim($dateDetail['name']).' '. print_date_daytime($dateDetail['date_time']). '. Click the button below to view '.$noun.' profile and chat with '.$pro_noun.':');
			...
			if($user_email = $this -> model_user -> get_user_email($from_user_id))
			{
	...
	public function match_date($id) {
		$this -> general -> set_table('user');
		$user_data = $this -> general -> get("user_id,first_name,gender_id,last_name,num_date_tix,mobile_phone_number", array('user_id' => $this -> user_id));
		$user = $user_data['0'];
		$data['user_data'] = $user;
		...
		if($data['date_info'] = $this -> general -> multijoins_arr($fields, $from, $joins, $where,$custom_where))
		{
			...
			if($applicant_user_id == $this->user_id)
			{
				...
				$this -> general -> set_table('user');
				$applicant_data = $this -> general -> get("user_id,gender_id,first_name,last_name,num_date_tix,mobile_phone_number", array('user_id' => $applicant_user_id));
			...
			if($host_user_id == $this->user_id)
			{
				...
				$this -> general -> set_table('user');
				$applicant_data = $this -> general -> get("user_id,gender_id,first_name,last_name,num_date_tix,mobile_phone_number", array('user_id' => $host_user_id));
	...
	public function view_date($date_id) {
		...
		if($data['date_info']['requested_user_id'] != $this -> user_id)
		{
			...
			if($applicant_user_data)
			{
				$data['applicant_user'] = $applicant_user_data['0'];	
			}
			else {
				// not have access to view date because user neigther a applicant nor a owner!
				$data['page_name'] = 'user/dates_app/message';
				$data['page_title'] = translate_phrase("view date");
				$data['msg'] = translate_phrase('Sorry! No such date (').$date_id .translate_phrase(') found with your profile').'.';	
				$this -> load -> view('template/editProfileTemplate', $data);
				$is_view_loaded = true;
			}
		}
		...
		$this -> general -> set_table('date_decision');
		if ($info = $this -> general -> get("count(date_decision_id) as total_views", array('date_id' => $data['date_info']['date_id']))) {
			$data['total_views'] = $info['0']['total_views'];
		}
		$this -> general -> set_table('date_applicant');
		if ($info = $this -> general -> get("count(date_applicant_id) as total_applications", array('date_id' => $data['date_info']['date_id'],'status !='=>3))) {
			$data['total_applications'] = $info['0']['total_applications'];
		}
		
		if($is_view_loaded == false)
		{
			$this -> db -> order_by('set_primary', 'desc');
			$this -> db -> select('*');
	...
	public function mutual_friend($fb_id, $other_user_id="") {
		...
		if ($friend_user_data = $this->general->get("user_id,first_name,facebook_id",array('facebook_id'=>$fb_id))) {
			$data['user_info'] = $friend_user_data['0'];
			...
			$this -> load -> view('user/dates_app/popup_mutual_friend', $data);
		}else {
