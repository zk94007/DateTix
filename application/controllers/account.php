...
class Account extends MY_Controller {
	...
	public function __construct() {
		...
		if ($this -> user_id = $this -> session -> userdata('user_id')) {
			...
			if ($this->uri->segment(1) != 'tix' && $this->uri->segment(2) != 'get-more-tickets.html')
			{
	...
	public function upgrade() {
		...
		if ($post_data = $this -> input -> post()) {
			$params['amount'] = isset($post_data['amount']) ? round($post_data['amount']) : '';
			...
			if ($response -> status() == Merchant_response::COMPLETE) {
				...
				if ($this -> general -> save($user_order)) {
					...
					if ($flag) {
                                                ...
                                                $this->general->set_table('user');
                                                $userDetail=$this->general->get('*',array('user_id'=>$this->user_id));
                                                
                                                $this->general->set_table('user_email');
                                                $userEmail=$this->general->get('*',array('user_email_id'=>$this->user_id));
                                                
                                                $name=$userDetail[0]['first_name'];
                                                $user_mail=$userEmail[0]['email_address'];
                                                
                                                // send upgrade account mail
                                                $upgrade_subject = translate_phrase("Your account has been upgraded");
												
						$email_template = $name.' , thanks for upgrading to a '.$post_data['no_of_unit'].' month premium membership';
	...
	public function get_more_tickets() {
		...
		if ($this -> user_id = $this -> session -> userdata('user_id')) {
			...
			echo '<script type="text/javascript">window.location.href = "' . base_url().url_city_name(). '/signin.html?return_url='.url_city_name().'/get-more-tickets.html"</script>';
			exit;
		}	
	...
	public function apply_fb_discount_on_tickets($is_like)
	{
		$this -> general -> set_table('user');
        $user_data = $this -> general -> get("fb_liked,num_date_tix", array('user_id' => $this -> user_id));
		$user = $user_data['0'];
		
		$update_data['num_date_tix'] = $user['num_date_tix'];
		$update_data['fb_liked'] = 1;
		
		$freeTickets = 500;
		$user_reward_data['user_id'] = $this->user_id;
		$user_reward_data['reward_id'] = 2;
		...
		if($is_like)
		{
			...
			if($user['fb_liked'] == 0)
			{
				$update_data['num_date_tix'] += $freeTickets;			
				$this -> general -> update($update_data, array('user_id' => $this -> user_id));
				
				$this->general->set_table('user_reward');
				$user_reward_data['reward_time'] = SQL_DATETIME;
				$user_reward_data['num_date_tix'] = $freeTickets;
				$this->general->save($user_reward_data);
			}
		}
		else {
			//if user action unlike page and fb_liked[db] = 1
			if($user['fb_liked'] == 1)
			{
				$update_data['num_date_tix'] -= $freeTickets;	
				$update_data['fb_liked'] = 0;
					
				$this -> general -> update($update_data, array('user_id' => $this -> user_id));				
				$this->general->set_table('user_reward');
				$this->general->delete($user_reward_data);
			}
		}
		
        $response['num_date_tix'] =  number_format($update_data['num_date_tix']);
		$response['type'] = 'success';		
		$response['msg'] = 'Discount applied.';
	...
	public function get_ticket_package() {
		...
		if ($ticket_packages) {
			...
			//$this->general->set_table('currency');
			...
			foreach ($ticket_packages as $key => $package) {
				...
				//$currency_condition['currency_id'] = $package['currency_id'];
				//$currency_condition['display_language_id'] = $this -> language_id;
				//$ticket_currency_data = $this->general->get("",$currency_condition);
				//$currency_id = $ticket_currency_data['0']['currency_id'];
				//$currency_rate = $ticket_currency_data['0']['rate'];
				//$currency_name = $ticket_currency_data['0']['description'];
												
				//Calculated per date price
				$ticket_packages[$key]['per_date_price'] = $ticket_packages[$key]['price'] / $ticket_packages[$key]['num_date_tix'];
				$ticket_packages[$key]['currency'] = $currency_name;
				
				
				$ticket_packages[$key]['per_date_price'] = $ticket_packages[$key]['per_date_price'] * $currency_rate;
				
				$ticket_packages[$key]['name'] = $ticket_packages[$key]['num_date_tix'];
				//$ticket_packages[$key]['total'] = $ticket_packages[$key]['price'] * $currency_rate;
				$ticket_packages[$key]['total'] = $ticket_packages[$key]['per_date_price'] * $ticket_packages[$key]['num_date_tix'];
				...
				$ticket_packages[$key]['description'] = number_format($ticket_packages[$key]['num_date_tix']);
                $ticket_packages[$key]['extra']='';
				if ($ticket_packages[$key]['num_date_tix'] == 5)
				{
					$ticket_packages[$key]['extra'] = translate_phrase('Most Popular!');
				}
				elseif ($ticket_packages[$key]['num_date_tix'] == 10)
				{
					$ticket_packages[$key]['extra'] = translate_phrase('Best Value!');
				}											
			}
		}
		//echo "<pre>";print_r($ticket_packages);
			...
			if ($ticket_packages) {
				foreach ($ticket_packages as $key => $package) {
					...
					if ($package['per_date_price']) {
						$package['per_date_price'] = $package['per_date_price'] * $discount;
					}
					
					if ($package['total']) {
						$package['total'] = $package['per_date_price'] * $package['num_date_tix'];
					}
	...
	public function success_payment() {
		if ($post_data = $this -> session -> userdata('post_data')) {
			...
			$params['amount'] = round($params['amount']);
			...
			if ($response -> status() == Merchant_response::COMPLETE) {
                            ...
                            // modify  by jigar for logged-in and non logged-in user
                            ...
                            if($this->user_id){
                                    ...
                                    $user_mail=$post_data['user_email'];
                                    
                                    $user_order['paid_by_name']=$post_data['first_name'];
                                    $user_order['paid_by_email']=$post_data['user_email'];
                                    $user_order['paid_by_phone']=$post_data['last_name'];
                                    $this -> general -> set_table('user_order');
                                    if($this -> general -> save($user_order)){
                                        $this -> session -> unset_userdata('post_data');
                                        $this -> session -> set_flashdata('paypal', $params['description'] . translate_phrase(' tickets have been credited to your account, Enjoy the dating!'));
	...
	public function settings() {
		...
		$where['membership.display_language_id'] = $this -> language_id;
                $where['membership.website_id'] = get_assets('website_id','0');
