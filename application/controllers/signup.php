...
class Signup extends MY_Controller {
	...
	public function step1()
	{
		...
		$user_email_info = $this->model_user->get_user_email($this->user_id);
		
		if ($postData = $this -> input -> post()) {
			
			if($this->input->post('email') && !$user_email_info)
			{
				$email = $this->input->post('email');
				if($this->general->checkDuplicate(array('email_address'=>$email),'user_email'))
				{
					$this->session->set_flashdata('email_error',translate_phrase('Email is already registered. Please use different email address.'));
					$this->session->set_flashdata('postData',$postData);
					
					redirect( base_url() . url_city_name()."/signup-step-1.html");
				}
				else {
					$this -> model_user -> insert_email	($this->user_id);
					$this -> model_user -> send_verification_mail($this->encrypt->encode($this->user_id));
				}
			}
		...
		$data['user_email_data'] = $user_email_info;
	...
	public function step4()
	{
		...
		$data['annual_income_range'] = array(); //$this -> model_user -> get_annual_income_range($country_id);
	...
	public function step7()
	{
		...
		if ($postData = $this -> input -> post()) {
            ...
            $reward_package = $this->general->get("",array('reward_id'=>$signup_reward_id));	
			
            $this->general->set_table('user_reward');
            $user_reward = $this->general->get("",array('reward_id'=>$signup_reward_id,'user_id'=>$this->user_id));
            
            
            if($user_data  && $reward_package && !$user_reward)
            {
            	//$user_data['0']['num_date_tix']+$reward_package[0]['num_date_tix']
        		$update_data['num_date_tix']= $reward_package[0]['num_date_tix'];
                
                $user_reward_data = array(
                        'user_id'=>$this->user_id,
                        'reward_id'=>$signup_reward_id,
                        'reward_time'=>SQL_DATETIME,
                        'num_date_tix'=>$reward_package[0]['num_date_tix'],
                );
                $this->general->set_table('user_reward');
                $this->general->save($user_reward_data);
				
				
				$log_user_date_ticket_data['user_id']= $this->user_id;
				$log_user_date_ticket_data['num_date_tickets']= $reward_package[0]['num_date_tix'];
				$log_user_date_ticket_data['transaction_time']= SQL_DATETIME;
				$log_user_date_ticket_data['description']= 'Signed Up Reward';
				$this->general->set_table('log_user_date_ticket');
                $this->general->save($log_user_date_ticket_data);
				
				$msg = translate_phrase("You have been given ".number_format($reward_package[0]['num_date_tix'])." free date tickets for signing up to DateTix! Use them to host dates, apply to dates, send gifts and more!");
				$this->session->set_userdata('reward_popup_msg',$msg);
            }
			
			$this->model_user->update_user($this->user_id,$update_data);
			
			$user_friends_with_datetix = array();//$this -> model_user -> get_fb_friends_with_datetix($this->user_id);
			if ($user_friends_with_datetix) {
				foreach ($user_friends_with_datetix as $friend) {
					...
					{
						$data['email_content'] = translate_phrase('One of your Facebook friends just applied to ').get_assets('name','DateTix').translate_phrase('. You now have ') . count($mutual_friend_on_datetix) . ' ' . translate_phrase('Facebook friends on DateTix who can introduce you to their friends for free!') . '</p><p class="lead">' . translate_phrase('Invite more of your friends to apply to ').get_assets('name','DateTix').translate_phrase(' to expand your own dating pool!');
