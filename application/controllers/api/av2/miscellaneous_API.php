...
class Miscellaneous_API extends MY_REST_Controller {
            ...
            $this -> response(array(
                'errors' => array(
                    array(
                        ...
                        'code' => 'Currently not supported',
                        'title' => 'Failed to process promotion code',
                        'detail' => 'You may not be given promotion code yet.'
                    )
                )
            ), 200);
        }
    ...
    public function get_my_info_get() {
        ...
        $my_upcoming_dates = $this -> date_api_model -> get_my_dates($this -> rest -> user_id, 0, 0, TRUE, TRUE);
    ...
    public function update_my_device_token_post() {

        $device_token = $this -> post('device_token');
        $device_token_android = $this -> post('device_token_android');

        if (!empty($device_token))
            $update_user_array['device_token'] = $device_token;

        if (!empty($device_token_android))
            $update_user_array['device_token_android'] = $device_token_android;
    ...
    public function update_my_location_post() {
