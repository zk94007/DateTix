...
class Membership_API extends REST_Controller {
    ...
    function __construct() {
        ...
        $this -> load -> model('api/av2/phone_verification_code_api_model');
        $this -> load -> model('api/av2/user_membership_option_api_model');
        $this -> load -> model('api/av2/user_api_model');
        $this -> load -> model('api/av2/user_chat_api_model');
    ...
    public function request_email_verification_code_post() {
            ...
            $this -> response(array(
                'errors' => array(
                    array(
                        ...
                        'code' => 'Currently not supported',
                        'title' => 'Failed to generate verification code',
                        'detail' => 'Email address is not provided.'
                    )
                )
            ), 200);
        }
        ...
        // Send email
        $subject = translate_phrase("Email Verification Code");
        $email_template = 'Here is a verification code from DateTix. Please Enter \'' . $verification_code . '\' to verify your email address.';
        $this -> model_user -> send_email(INFO_EMAIL,$email_address, $subject , $email_template,"html","DateTix");

        $exists = $this -> user_api_model -> is_email_address_exists($email_address);
    ...
    public function sign_in_post() {
        ...
        $my_upcoming_dates = $this -> date_api_model -> get_my_dates($user_id, 0, 0, TRUE, TRUE);
        $my_upcoming_dates_count = count($my_upcoming_dates);

        $followed_date_ids = $this -> user_follow_date_api_model -> get_followed_date_ids_by_user_id($user_id);
        $my_follows_dates_count = count($followed_date_ids);

        $user_follow_users = $this -> user_follow_user_api_model -> get_user_follow_users_by_user_id($user_id);
        $my_follows_people_count = count($user_follow_users);

        $followed_merchant_ids = $this -> user_follow_merchant_api_model -> get_followed_merchant_ids_by_user_id($user_id);
        $my_follows_merchants_count = count($followed_merchant_ids);

        $user_visit_users = $this -> user_visit_user_api_model -> get_user_visit_users_by_visited_user_id($user_id);
        $my_visitors_count = count($user_visit_users);

        $user_public_profile_count = $this -> user_api_model -> get_user_public_profile_count($user_id);
        $user_experience_count = $this -> user_api_model -> get_user_experience_count($user_id);
        $user_personality_count = $this -> user_api_model -> get_user_personality_count($user_id);
        ...
        $this -> response(array(
            'data' => array(
                ...
                'meta' => array(
                    ...
                    'my_follows_dates_count' => $my_follows_dates_count,
                    'my_follows_people_count' => $my_follows_people_count,
                    'my_follows_merchants_count' => $my_follows_merchants_count,
    ...
    public function facebook_sign_in_post() {
        ...
        if ($this -> user_api_model -> is_facebook_id_exists($facebook_id)) {
            ...
            $my_upcoming_dates = $this -> date_api_model -> get_my_dates($user -> user_id, 0, 0, TRUE, TRUE);
            $my_upcoming_dates_count = count($my_upcoming_dates);

            $followed_date_ids = $this -> user_follow_date_api_model -> get_followed_date_ids_by_user_id($user -> user_id);
            $my_follows_dates_count = count($followed_date_ids);

            $user_follow_users = $this -> user_follow_user_api_model -> get_user_follow_users_by_user_id($user -> user_id);
            $my_follows_people_count = count($user_follow_users);

            $followed_merchant_ids = $this -> user_follow_merchant_api_model -> get_followed_merchant_ids_by_user_id($user -> user_id);
            $my_follows_merchants_count = count($followed_merchant_ids);

            $user_visit_users = $this -> user_visit_user_api_model -> get_user_visit_users_by_visited_user_id($user -> user_id);
            $my_visitors_count = count($user_visit_users);

            $user_public_profile_count = $this -> user_api_model -> get_user_public_profile_count($user -> user_id);
            $user_experience_count = $this -> user_api_model -> get_user_experience_count($user -> user_id);
            $user_personality_count = $this -> user_api_model -> get_user_personality_count($user -> user_id);
            $profile_completeness_percent = intval(($user_public_profile_count + $user_experience_count + $user_personality_count) / (9 + 4 + 4) * 100);
            if ($profile_completeness_percent > 100) {
                $profile_completeness_percent = 100;
            }
