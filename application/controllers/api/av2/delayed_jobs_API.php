...
class Delayed_Jobs_API extends  REST_Controller {
    ...
    // Test purpose
    public function send_new_date_push_notifications_to_my_followers_post() {
        ...
        $this -> send_new_date_push_notifications_to_my_followers(json_encode($params));
    }

    private function send_new_date_push_notifications_to_my_followers($params_str) {
        ...
        $user_follow_users = $this -> user_follow_user_api_model -> get_user_follow_users_by_follow_user_id($user_id);

        if (!empty($user_follow_users)) {

            $message = "{$user -> first_name} has posted a date at {$merchant -> name}";
            $meta = array(
                'notification_type' => 'new_date_from_your_followed_user',
                'date_id' => $date_id
            );
            ...
            foreach ($user_follow_users as $user_follow_user) {
                ...
                $this -> user_api_model -> send_push_notification_to_user($user_follow_user['user_id'], $message, $meta);
            }
        }
    }

    // Test purpose
    public function send_new_date_push_notifications_to_merchant_followers_post() {
        ...
        $this -> send_new_date_push_notifications_to_merchant_followers(json_encode($params));
    }

    private function send_new_date_push_notifications_to_merchant_followers($params_str) {
        ...
        $user_follow_merchants = $this -> user_follow_merchant_api_model -> get_user_follow_merchants_by_merchant_id($merchant -> merchant_id);

        if (!empty($user_follow_merchants)) {

            $message = "{$user -> first_name} has posted a date at {$merchant -> name}";
            $meta = array(
                'notification_type' => 'new_date_at_your_followed_merchant',
                'date_id' => $date_id
            );

            foreach ($user_follow_merchants as $user_follow_merchant) {

                // Send push notification
                $this -> user_api_model -> send_push_notification_to_user($user_follow_merchant['user_id'], $message, $meta);
            }
        }
    }

    // Test purpose
    public function send_cancelled_date_push_notifications_to_date_followers_post() {
        ...
        $this -> send_cancelled_date_push_notifications_to_date_followers(json_encode($params));
    }

    private function send_cancelled_date_push_notifications_to_date_followers($params_str) {
        ...
        $following_user_ids = $this -> user_follow_date_api_model -> get_following_user_ids_by_date_id($date -> date_id);

        if (!empty($following_user_ids)) {

            $message = "{$user -> first_name} has cancelled date at {$merchant -> name}";
            $meta = array(
                'notification_type' => 'cancelled_date_from_your_followed_date',
                'date_id' => $date_id
            );
    ...
    // Test purpose
    public function send_cancelled_date_push_notifications_to_date_applicants_post() {
        ...
        $this -> send_cancelled_date_push_notifications_to_date_applicants(json_encode($params));
    }

    private function send_cancelled_date_push_notifications_to_date_applicants($params_str) {
        ...
        $applicant_user_ids = $this -> date_applicant_api_model -> get_applicant_user_ids_by_date_id($date -> date_id);

        if (!empty($applicant_user_ids)) {

            $message = "{$user -> first_name} has cancelled date at {$merchant -> name}";
            $meta = array(
                'notification_type' => 'cancelled_date_from_your_applied_date',
                'date_id' => $date_id,
                'is_upcoming_date' => $date_time -> isFuture()
            );

            foreach ($applicant_user_ids as $applicant_user_id) {

                // Send push notification
                $this -> user_api_model -> send_push_notification_to_user($applicant_user_id, $message, $meta);
            }
        }
    }
