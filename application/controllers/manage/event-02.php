...
class Event extends CI_Controller {
	...
	public function index()
	{
		...
		$fields = array('e.event_id, e.event_name, e.event_start_time, e.price_door, e.ticket_sold_at_door, e.cash_collected_at_door, e.price_online, e.price_online_discounted',
						...
						'SUM(event_order.num_tickets) as online_prepaid_tkt',
						'SUM(event_order.order_amount) as online_prepaid_amt',						
						'currency.currency_id ,currency.description as currency_name'
						);
		$from = 'event as e';
		$joins = array(
				'event_order' => array('event_order.event_id = e.event_id', 'left'), 
				'currency' => array('currency.currency_id = event_order.currency_id', 'left'),
		...
		//$where['currency.display_language_id'] = $this -> language_id;		
	...
	public function create($event_id="")
	{
		...
		{
			//Get Event datas
			$fields = array('e.event_id, e.event_name, e.event_start_time, e.price_door, e.ticket_sold_at_door, e.cash_collected_at_door, e.price_online, e.price_online_discounted','ct.city_id, ct.description as city_name');
			$from = 'event as e';
			$joins = array(
					'venue as v' => array('e.venue_id = v.venue_id', 'inner'), 
					'neighborhood as n' => array('v.neighborhood_id = n.neighborhood_id', 'inner'),
			...
			if($city_ids)
			{
				$this->db->where_in('ct.city_id',$city_ids);
			}
		...
		$this->db->where_in('c.city_id',$city_ids);
