	...
	public function details() {
		...
		else
		{
			$this -> session -> unset_userdata('ad_id');
                ...
                $urls_id = NULL;		
		if ($url_id = $this -> input -> get('url')) {
                    $data['form_para'] .= '&url='.$url_id;
                    $urls_id = $url_id;
                    $this -> session -> set_userdata('url', $url_id);				
                        }
		else
		{
		...
		if (isset($getParams['partner_id'])) {
	            $partner_id = $this -> input -> get('partner_id');
	            $data['form_para'] .= '&partner_id='.$partner_id;
				$data['partner_path'] = 'partner_id='.$partner_id;
		...
		if($event_data = $this->_get_event_details())
		{
			if($event_id = $this -> session -> userdata('event_id'))
			{
				...
				$event_data['partner_id'] = $partners_id;
				
				$this->general->set_table("log_event_ad");
...
//			else
			...
			if($user)
			{
				if($user['completed_application_step'] >= 5 )
				{
...
//							else
	...
	private function _get_ticket_package($event_data) {
		...
		//$package_price = $this->user_id?$event_data['price_online_discounted'] - $event_data['partner_discount'] : $event_data['price_online'] - $event_data['partner_discount'];
		
		$price_online = $event_data['price_online'] -  $event_data['partner_discount'];
		$price_online_discounted = $event_data['price_online_discounted'] -  $event_data['partner_discount'];
		$actual_price = $this->user_id?$price_online_discounted: $price_online;
								
		$total_save_amount = $price_online - $price_online_discounted;
		$save_per = round(100 - ($price_online_discounted * 100) / $price_online);
		...
		$ticket_packages['0'] = array(
            ...
            'per_date_price'=>$actual_price, //Discounted price
            'total'=>$actual_price,
            'name' => '1', //Package Name
	...
	private function _get_event_details()
	{
		...
		$fields = array('v.*','e.*','el.event_name','el.description','el.flyer_url',
		...
		if($event_data = $this -> model_user -> multijoins($fields, $from, $joins, $where,'','e.event_start_time asc'))
		{
			...
			{
				$url_segment = $post_data['url'];
			}
			else
			{
				$url_segment = $this->input->get('url');
			}
			...
			if($url_segment != "")
			{
				...
				$event_info['url'] = $url_segment;
			}
			$event_info['partner_discount'] = $partner_discount;
			return $event_info;
		}
		else
		{
	...
	*/
	public function event_photos($event_id = 1)
	{
		...
		$fields = array('v.*','e.*','el.event_name',
						'n.description as neighborhood_name','ct.description as city_name',
						'p.description as province_name','c.description as country_name'
						);
		$from = 'event as e';
