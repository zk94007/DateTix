...
class My_intros extends MY_Controller {
	...
	public function test() {
		//common SQL Query for all the intros
		 $common_sql = 'SELECT user_intro.*, MAX(user_intro_chat.user_intro_chat_id) as user_intro_chat_id,
			 ...
			 $query = 'SELECT user_intro.*, MAX(user_intro_chat.user_intro_chat_id) as user_intro_chat_id,
				user.first_name as intro_name,user.gender_id, user.ethnicity_id, user.body_type_id, user.privacy_photos,
				user.user_id, user.facebook_id, user.birth_date, user.current_city_id,
				CASE
					WHEN
						birth_date != "0000-00-00" 	
				...
				LEFT JOIN user_intro_chat on user_intro_chat.user_intro_id = user_intro.user_intro_id
				JOIN user on user.user_id = CASE 
					WHEN user_intro.user1_id = "446" THEN user_intro.user2_id
					WHEN user_intro.user2_id = "446" THEN user_intro.user1_id
				END
				WHERE (user1_id = "446" OR user2_id = "446")  AND handpicked_time IS NOT NULL group by user_intro.user_intro_id
                ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC  LIMIT 0, 10';
...
echo $query;
	...
	public function index($intro_type = 'active') {
        ...
        if($intro_type == 'handpicked'){
           $query = $common_sql . ' AND (handpicked_time IS NOT NULL AND handpicked_time != "0000-00-00 00:00:00") group by user_intro.user_intro_id
                ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC ' . $limit;
		
        }	
        else if ($intro_type == 'expired') {
            $query = $common_sql . 'AND DATE(intro_expiry_time) < DATE(CURDATE()) AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00") group by user_intro.user_intro_id
                ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC ' . $limit;

            
        } elseif ($intro_type == 'upcoming') {
            $query = $common_sql . 'AND DATE(intro_expiry_time) >= DATE(CURDATE()) AND DATE(intro_available_time) > DATE(CURDATE()) AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00") group by user_intro.user_intro_id
                ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC ' . $limit;
           
        } else {
            $query = $common_sql . 'AND DATE(intro_expiry_time) >= DATE(CURDATE()) AND DATE(intro_available_time) <= DATE(CURDATE()) AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00") group by user_intro.user_intro_id ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC' . $limit;
            //$query = $common_sql . 'group by user_intro.user_intro_id ORDER BY user_intro_chat_id DESC, `intro_available_time` DESC' . $limit;
...
$intros_data = $this->general->sql_query($query);
 	//echo $query;
        ...
        if ($intros_data) {
            foreach ($intros_data as $intro) {
                ...
                //if expired or not [Differenciate all intros]
                //echo '<pre>';print_r($intro);die(); 
                if($intro['handpicked_time'] != '0000-00-00 00:00:00'){
                   $result['handpicked_data'][] = $tmp_intro;
                }
                else if (date("Y-m-d", strtotime($intro['intro_expiry_time'])) < SQL_DATE) {
                        $result['expired'][] = $tmp_intro;
                } else {
                    if (date("Y-m-d", strtotime($intro['intro_available_time'])) > SQL_DATE) {
                	//echo '<pre>';print_r($tmp_intro);die(); 
                        $result['upcoming'][] = $tmp_intro;
                    } else {
                        $result['active'][] = $tmp_intro;
                    }
	    ...
	    //echo '<pre>';print_r($intro_type);die();
	    //echo '<pre>';print_r($data);die();
	...
	public function load_intro($intro_type = 'active') {
        ...
        if ($sortby == 1) {
            $sort_order = 'ORDER BY `user_intro_chat_id` DESC';
        } elseif ($sortby == 2) {
            $sort_order = 'ORDER BY `user_intro_chat_id` ASC';
        } elseif ($sortby == 3) {
        ...
        if ($intro_type == 'handpicked') {
            $query = $common_sql . ' AND (handpicked_time IS NOT NULL AND handpicked_time != "0000-00-00 00:00:00")';
        }
        else if ($intro_type == 'expired') {
            $query = $common_sql . 'AND DATE(intro_expiry_time) < DATE(CURDATE()) AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00")';
        } else if ($intro_type == 'upcoming') {
            $query = $common_sql . 'AND DATE(intro_expiry_time) >= DATE(CURDATE()) AND DATE(intro_available_time) > DATE(CURDATE()) AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00")';
        } else {
            $query = $common_sql . 'AND DATE(intro_expiry_time) >= DATE(CURDATE()) AND DATE(intro_available_time) <= DATE(CURDATE()) AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00")';
        }

        /* [Types three category] 
          if ($intro_type == 'upcoming') {
        ...
        if ($intros_data) {
            foreach ($intros_data as $intro) {
                ...
                elseif (date("Y-m-d", strtotime($intro['intro_expiry_time'])) < SQL_DATE) {
                    $result['expired'][] = $tmp_intro;
                } else {
                    if (date("Y-m-d", strtotime($intro['intro_available_time'])) > SQL_DATE) {
                        $result['upcoming'][] = $tmp_intro;
