...
class partner extends CI_Controller {
	 ...
	 */
	 public function view_ad_details($franchise="")
	 {
		...
		*/
		...
		if($ad_id )
		{
			$user_sql = 'SELECT u.user_id, u.first_name, u.last_name, u.gender_id, u.mobile_phone_number, ue.email_address, u.applied_date, u.completed_application_step FROM user u LEFT JOIN user_email ue ON u.user_id=ue.user_id WHERE u.partner_id='.$ad_id.' GROUP BY u.user_id  ORDER BY applied_date DESC';
			$data['users'] = $this->general->sql_query($user_sql );
			
			$member_sql = 'SELECT currency.description as currency_name, eo.event_id, et.event_ticket_id, eo.order_amount, u.user_id, u.first_name, u.last_name, u.gender_id, u.mobile_phone_number, ue.email_address, eo.order_time 
			FROM event_order eo 			
			JOIN currency ON currency.currency_id = eo.currency_id
			JOIN event_ticket et ON (eo.event_order_id=et.event_order_id) 
			JOIN user u ON (eo.paid_by_user_id=u.user_id) 
			JOIN user_email ue ON (u.user_id=ue.user_id) 
			
			WHERE 
			currency.display_language_id = "'.$this->language_id.'" AND
			u.partner_id='.$ad_id.' 
			ORDER BY et.event_ticket_id DESC;';
			$data['member_users'] = $this->general->sql_query($member_sql );
				
			$non_member_sql = 'SELECT eo.event_id, eo.order_amount, eo.order_time, eo.paid_by_name, eo.paid_by_mobile_phone_number, eo.paid_by_email, et.event_ticket_id,
			...
			WHERE 
			currency.display_language_id = "'.$this->language_id.'" AND
			eo.paid_by_user_id is NULL AND eo.ad_id='.$ad_id.' ORDER BY et.event_ticket_id DESC';
			$data['non_member_users'] = $this->general->sql_query($non_member_sql);
			
			//echo "<pre>";print_r($data['non_member_users']);exit;
			$data['franchise'] = $franchise;	 	
		}
		else {
	...
	public function test($ad_id=0)
	{
		
		$member_sql = 'SELECT first_name,last_name,partner_id,from_url,applied_date FROM user order by applied_date desc limit 10;';
		echo $member_sql;
			$data['member_users'] = $this->general->sql_query($member_sql );
	 ...
	 */
	 public function find_match($ad_id=0,$user_id=1)
	 {
		...
		$ad_id = $franchise_data['partner_id'];
	 	...
	 	$data['franchise'] = $franchise_data['name'];
