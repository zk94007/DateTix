...
// Following code is use for find country name from database if query string and database country name is same
...
$db =& DB();
/*
$city_name = my_custom_segment(0);
$query = $db->get_where('city',"LOWER(REPLACE(description,' ','')) = '".$city_name."'");
if($query->num_rows)
{
	$route[$city_name] = 'home/index';
}

$language_mapping['1'] = 'english';
$language_mapping['2'] = 'chinese_t';
$language_mapping['3'] = 'chinese';		
if($lang_id = array_search($first_segment, $language_mapping))
{
	$route[$first_segment] = 'home';
}
 */
 
/* when user enter datetix.com/xxx
1. check landing_page table
2a. if found, then stop
2b. if not found, then check test table
3a. if found, then stop
3b. if not found, then redirect to datetix.com
*/  
//Find Website_id 
...
$website_data = array();
$website_id = 0;
/* [Website landing page - off]*/
if($query = $db->get_where("website",array('domain'=>$domain_name)))
{
	$website_data = $query->result_array();
	
	$website_id = isset($website_data['0']['website_id'])?$website_data['0']['website_id']:0; 
}

//echo "<pre>";print_r($website_id);exit;
//echo "<pre>";print_r(my_custom_segment(0));exit;
$first_segment = my_custom_segment(0);
if (my_custom_segment(0) != "/") {	
        
	$query = $db->get_where('landing_page',"LOWER(REPLACE(shortcut_url,' ','')) = '".$first_segment."'");
}
elseif($website_id)
{
	$where = "website_id REGEXP '[[:<:]]".$website_id."[[:>:]]'";	
	$query = $db->get('landing_page',$where);
...
if($query->num_rows)
{
	$route['(:any)'] = 'home/load_landing_page/$1';
}
else { 
	$query = $db->get_where('test',"LOWER(REPLACE(url,' ','')) = '".$first_segment."'");
	if($query->num_rows)
	{
		$route[$first_segment] = 'test/test_details/$1';
	}

	$query = $db->get_where('signup_url',"LOWER(REPLACE(url,' ','')) = '".$first_segment."'");	
	if($query->num_rows)
	{
		$route[$first_segment]  = 'home/shortcut_url';
	}
	
	$query = $db->get_where('event_url',"LOWER(REPLACE(url,' ','')) = '".$first_segment."'");
	if($query->num_rows)
	{
		$route[$first_segment]  = 'home/shortcut_url';
	}
	
	$query = $db->get_where('event',"LOWER(REPLACE(shortcut_url,' ','')) = '".$first_segment."'");
	if($query->num_rows)
	{
		$route[$first_segment]  = 'home/shortcut_url';
	}
    if(strtolower($first_segment)=='love'){
        $route[$first_segment]  = 'account/get_more_tickets';
    }

    if(strtolower($first_segment)=='upgrade_account'){
        $route[$first_segment]  = 'upgrade_account';
    }
    if(strtolower($first_segment)=='upgrade'){
        $route[$first_segment]  = 'account/upgrade';
    }
}
