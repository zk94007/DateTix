...
class Datetix {
	...
	/**
	 * Destroy session : Unset data from current session
	 * @access public
	 * @return true or false
	 * @author Rajnish
	 */
	function destroy_current_session($sess_array_keys = array())
	{
		if(!$sess_array_keys)
			$sess_array_keys = array('fb_user_data','is_return_apply','sign_up_id','user_id','extra_para','user_data');
		
		foreach($sess_array_keys  as $key)
		{
	...
	public function mail_to_user_debug($to,$subject,$message)
	{
		...
		$user_website_data = $this->CI->model_user->get_website_by_user_email($to);		
		...
		$from_email = isset($user_website_data['from_email_address'])?$user_website_data['from_email_address']:$from;
		$from_text = isset($user_website_data['from_email_name'])?$user_website_data['from_email_name']:'DateTix';
		
		$message = str_replace('WEBSITE_LOGO', $logo_url, $message);
		$message = str_replace('WEBSITE_NAME', $website_name, $message);
		
		$this->CI->email->message($message);
		$this->CI->email->from($from_email, $from_text);
		$this->CI->email->to($to);
	...
	public function is_premium_user($user_id,$membership_option_id)
	{
		...
		if(is_numeric($user_id) && is_numeric($membership_option_id))
		{
	...
	public function intro_attendance_upcoming_event($intro_id,$gender_id=1)
	{
		...
		$fields = array('e.event_id','el.event_name','e.event_start_time');
		$from = 'event as e';
		$joins = array(
			'event_language as el' => array('el.event_id = e.event_id', 'left'),				
			'event_user as eu' => array('eu.event_id  = e.event_id', 'INNER')
		);
	...
	public function load_data_by_id($p_table,$fk=null)
	{
		if($fk)
		{
			...
			$get_condition['display_language_id'] = $this->language_id;
	...
	public function datetix_mutual_friend_count($user_id,$friend_id)
	{
		$user_friends_with_datetix = $this->CI->model_user->get_fb_friends_with_datetix($user_id);
		$cmp_user_friends_with_datetix = $this->CI->model_user->get_fb_friends_with_datetix($friend_id);
	...
	function intro_mail($user_id=3, $intro_id=1, $match_criteara = array('looks'),$mail_to= '')
	{
		...
		if($intro_user['gender_id'] == 1)
		{
			...
			$pro_noun = 'his';
			$data['vuserfits'] = translate_phrase('He fits');
			$data['vuser_looking_for'] = translate_phrase('of what he is looking for');
		}
		else
		{
			...
			$data['vuserfits'] = translate_phrase('She fits');
			$data['vuser_looking_for'] = translate_phrase('of what she is looking for');
		}
		$subject_line = '';
		...
		if($this->CI->session->userdata('type') == 'expire_intro_reminder')
		{
			...
			$data['first_line'] = translate_phrase('Your introduction with ').$intro_user['first_name'].translate_phrase(' is expiring in less than 24 hours');
                ...
                else if($set_intros_data['handpicked_time'] != NULL && $set_intros_data['handpicked_time']!='0000-00-00 00:00:00')
		{
			//$subject = $user['first_name'].', meet '.$intro_user['first_name'].', '.$age_string.', '.$subject_line ;
                        $subject = $user['first_name'].', meet '.$intro_user['first_name'].', our handpicked match for you!' ;
		...
		$data['backLink'] = base_url().'user/user_info/'.$this->CI->utility->encode($intro_user['user_id']).'/'.$this->CI->utility->encode($user['user_id']).'/'.$user['password'].'?redirect_intro_id='.$set_intros_data['user_intro_id'];
		
		$data['email'] = $user_email_data['email_address'];
		//$data['email'] = 'mikeye27@gmail.com';
		
		$data['cur_usr_demand_cmp'] = $this -> calculate_score($user_id, $intro_id);
		$data['view_usr_demand_cmp'] = $this -> calculate_score($intro_id, $user_id);
		
		//$data['match_with_intro'] = $match_criteara;
		$data['match_with_intro'] = $data['cur_usr_demand_cmp']['match_data'];
		...
		$this->CI->general->set_table('user_intro');
		if($is_intro_already_exist = $this->CI -> general -> custom_get("*", '(user1_id = "' . $user_id . '" AND user2_id = "' . $intro_id . '") OR (user1_id = "' . $intro_id . '" AND user2_id = "' . $user_id . '") AND (handpicked_time IS NULL OR handpicked_time = "0000-00-00 00:00:00")'))
		{
			
			$this->CI->general->update(array('intro_email_sent_time'=>SQL_DATETIME),array('user_intro_id'=>$is_intro_already_exist['0']['user_intro_id']));
			$this->CI->general->update(array('intro_expiry_time'=>calculate_current_time($user_city -> time_zone_val, "+200 day")),array('user_intro_id'=>$is_intro_already_exist['0']['user_intro_id']));
			//if($this->CI->general->update(array('intro_email_sent_time'=>SQL_DATETIME),array('user_intro_id'=>$is_intro_already_exist['0']['user_intro_id'])))
			//{
				//update last_last_email_sent
				$this->CI->general->set_table('user');
				$this->CI->general->update(array('last_intro_mail_sent'=>SQL_DATETIME),array('user_id'=>$user['user_id']));
