...
abstract class PayPal_Digital_Goods {
	...
	public function __construct($args = array()) {
		...
		//MY TEST DATA
		/*
		PayPal_Digital_Goods_Configuration::username($this->CI -> config->item('username'));
		PayPal_Digital_Goods_Configuration::password($this->CI -> config->item('password'));
		PayPal_Digital_Goods_Configuration::signature($this->CI -> config->item('signature'));
		if($this->CI -> config->item('test_mode'))
		{
			PayPal_Digital_Goods_Configuration::environment('sandbox');	
		}
		*/
		PayPal_Digital_Goods_Configuration::username('rajnish-facilitator_api1.ncodetechnologies.com');
		PayPal_Digital_Goods_Configuration::password('1391763734');
		PayPal_Digital_Goods_Configuration::signature('AN8cmhiVv-RVEViZfoHtcU1Gy5sjAHbBq6k3GZ.x5GT6IKTlMWZMbi9-');
		PayPal_Digital_Goods_Configuration::environment('sandbox');	
		//END MY TEST DATA

		PayPal_Digital_Goods_Configuration::return_url($args['return_url']);
		PayPal_Digital_Goods_Configuration::cancel_url($args['cancel_url']);
		PayPal_Digital_Goods_Configuration::business_name('Demo Store');
		PayPal_Digital_Goods_Configuration::notify_url($args['notify_url']);
		...
		elseif ((empty($args['return_url']) && '' == PayPal_Digital_Goods_Configuration::username()) || (empty($args['cancel_url']) && '' == PayPal_Digital_Goods_Configuration::cancel_url()))
			...
			$defaults = array('sandbox' => false, 'business_name' => '', 'solution_type' => 'Sole', 'return_url' => PayPal_Digital_Goods_Configuration::return_url(), 'cancel_url' => PayPal_Digital_Goods_Configuration::cancel_url(), 'notify_url' => PayPal_Digital_Goods_Configuration::notify_url(), 'locale_code' => PayPal_Digital_Goods_Configuration::locale_code(), // Defaults to 'US'
		);
	...
	protected function get_api_credentials_url() {
		...
		return 'USER=' . urlencode(PayPal_Digital_Goods_Configuration::username()) . '&PWD=' . urlencode(PayPal_Digital_Goods_Configuration::password()) . '&SIGNATURE=' . urlencode(PayPal_Digital_Goods_Configuration::signature()) . '&VERSION=' . urlencode(PayPal_Digital_Goods_Configuration::version());
	}
	...
	protected function get_payment_details_url($action, $profile_or_transaction_id = '') {
		...
		if ('SetExpressCheckout' == $action) {
			...
			$api_request .= '&METHOD=SetExpressCheckout' . '&RETURNURL=' . urlencode($this -> return_url) . '&SOLUTIONTYPE=' . urlencode($this -> solution_type) . '&LOCALECODE=' . urlencode($this -> locale_code) . '&CANCELURL=' . urlencode($this -> cancel_url);
	...
	public function get_buy_button($args = array()) {
		...
		$defaults = array('id' => 'paypal-submit', 'type' => 'anchor', 'href' => PayPal_Digital_Goods_Configuration::checkout_url(), 'alt' => 'Submit', 'get_token' => true, 'locale' => 'en_US');
		...
		if ($args['type'] == 'anchor') {
			...
			$button = '<a href="' . $args['href'] . '" id="' . $args['id'] . '" alt="' . $args['alt'] . '"><img style="width:100%;" src="' . base_url() . 'assets/home/images/PAYPAL_image_YS9.jpg" border="0" /></a>';
		} else {
