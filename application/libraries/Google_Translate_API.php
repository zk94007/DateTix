...
class Google_Translate_API {
        ...
        function translate($text, $from = '', $to = 'en') {
            $key = '';
            $url = 'https://www.googleapis.com/language/translate/v2?key='.$key.'&q='.rawurlencode($text).'&source='.rawurlencode($from).'&target='.rawurlencode($to);
			...
			$newvar    = json_decode($response);
			if($newvar)
			{
			if ($newvar->data->translations[0]->translatedText) {
			   return $newvar->data->translations[0]->translatedText;
            }	
			}
